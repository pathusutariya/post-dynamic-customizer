(function($, undefined){
	...
	*/
	...
	var fieldGroupManager = new pdc.Model({
		...
		onSubmit: function( e, $el ){
			...
			if( !$title.val() ) {
				...
				// alert
				alert( pdc.__('Field group title is required') );
		...
		onClickTrash: function( e ){
			var result = confirm( pdc.__('Move to trash. Are you sure?') );
			if( !result ) {
				e.preventDefault();
			}
		},
	...
	*/
	...
	var screenOptionsManager = new pdc.Model({
		...
		initialize: function(){
			...
			var $append = $('#pdc-append-show-on-screen');
		...
		onChange: function( e, $el ) {
			...
			pdc.updateUserSetting('show_field_keys', val);
			this.render();
		},
		
		render: function(){
			if( this.isChecked() ) {
				$('#pdc-field-group-fields').addClass('show-field-keys');
			} else {
				$('#pdc-field-group-fields').removeClass('show-field-keys');
			}
		}
	...
	*/
	...
	var appendFieldManager = new pdc.Model({
		...
		onNewField: function( field ){
			...
			// ul
			var $div = $sibling.children('.pdc-input');
			var $ul = $div.children('ul');
			
			// create ul
			if( !$ul.length ) {
				$div.wrapInner('<ul class="pdc-hl"><li></li></ul>');
				$ul = $div.children('ul');
			}
			
			// li
			var html = field.$('.pdc-input').html();
			var $li = $('<li>' + html + '</li>');
			$ul.append( $li );
...
(function($, undefined){
	...
	pdc.FieldObject = pdc.Model.extend({
		
		// class used to avoid nested event triggers
		eventScope: '.pdc-field-object',
		...
		$setting: function( name ){
			return this.$('.pdc-field-settings:first > .pdc-field-setting-' + name);
		},
		
		getParent: function(){
			return pdc.getFieldObjects({ child: this.$el, limit: 1 }).pop();
		},
		
		getParents: function(){
			return pdc.getFieldObjects({ child: this.$el });
		},
		...
		render: function(){
			...
			// update menu order
			$handle.find('.pdc-icon').html( parseInt(menu_order) + 1 );
			...
			pdc.doAction('render_field_object', this);
		},
		
		refresh: function(){
			pdc.doAction('refresh_field_object', this);
		},
		...
		open: function(){
			...
			// action (open)
			pdc.doAction('open_field_object', this);
			this.trigger('openFieldObject');
			
			// action (show)
			pdc.doAction('show', $settings);
		},
		...
		close: function(){
			...
			// action (close)
			pdc.doAction('close_field_object', this);
			this.trigger('closeFieldObject');
			
			// action (hide)
			pdc.doAction('hide', $settings);
		},
		
		serialize: function(){
			return pdc.serialize( this.$el, this.getInputName() );
		},
		...
		save: function( type ){
			...
			// action
			pdc.doAction('save_field_object', this, type);
		},
		...
		submit: function(){
			...
			// action
			pdc.doAction('submit_field_object', this);
		},
		...
		onChange: function( e, $el ){
			...
			// action for 3rd party customization
			pdc.doAction('change_field_object', this);
		},
		...
		onChanged: function( e, $el, name, value ){
			...
			// action for 3rd party customization
			pdc.doAction('change_field_object_' + name, this, value);
		},
		...
		onChangeLabel: function( e, $el ){
			...
			// render name
			if( this.prop('name') == '' ) {
				var name = pdc.applyFilters('generate_field_object_name', pdc.strSanitize(label), this);
				this.prop('name', name);
			}
		},
		...
		onChangeName: function( e, $el){
			...
			if( name.substr(0, 6) === 'field_' ) {
				alert( pdc.__('The string "field_" may not be used at the start of a field name') );
			}
		},
		...
		delete: function( args ){
			...
			if( id ) {
				var $input = $('#_pdc_delete_fields');
				var newVal = $input.val() + '|' + id;
				$input.val( newVal );
			}
			
			// action
			pdc.doAction('delete_field_object', this);
		...
		removeAnimate: function(){
			...
			var $fields = pdc.findFieldObjects({
				sibling: this.$el
			});
			...
			pdc.remove({
				...
				complete: function(){
					field.remove();
					pdc.doAction('removed_field_object', field, $list);
				}
			});
			
			// action
			pdc.doAction('remove_field_object', field, $list);
		},
		
		duplicate: function(){
			
			// vars
			var newKey = pdc.uniqid('field_');
			
			// duplicate
			var $newField = pdc.duplicate({
				target: this.$el,
				search: this.get('id'),
				replace: newKey,
			});
			...
			// get instance
			var newField = pdc.getFieldObject( $newField );
			...
			// action
			pdc.doAction('duplicate_field_object', this, newField);
			pdc.doAction('append_field_object', newField);
		},
		...
		wipe: function(){
			...
			// action
			pdc.doAction('wipe_field_object', this);
		},
		...
		move: function(){
			...
			if( changed ) {
				alert( pdc.__('This field cannot be moved until its changes have been saved') );
				return;
			}
			...
			var step1 = function(){
				...
				// popup
				popup = pdc.newPopup({
					title: pdc.__('Move Custom Field'),
					loading: true,
					width: '300px'
				});
				...
				// ajax
				var ajaxData = {
					action:		'pdc/field_group/move_field',
					field_id:	id
				};
			...
			var step3 = function( e, $el ){
				...
				// disable
				pdc.startButtonLoading( popup.$('.button') );
		...
		changeType: function( newType ){
			...
			var prevClass = pdc.strSlugify( 'pdc-field-object-' + prevType );
			var newClass = pdc.strSlugify( 'pdc-field-object-' + newType );
			...
			// load settings
			var $loading = $('<tr class="pdc-field"><td class="pdc-label"></td><td class="pdc-input"><div class="pdc-loading"></div></td></tr>');
			this.$setting('conditional_logic').before( $loading );
			
			// ajax
			var ajaxData = {
				action: 'pdc/field_group/render_field_settings',
...
(function($, undefined){
	...
	*/
	...
	var modelId = function( type ) {
		return pdc.strPascalCase( type || '' ) + 'FieldSetting';
	};
	...
	*/
	...
	pdc.registerFieldSetting = function( model ){
	...
	*/
	...
	pdc.newFieldSetting = function( field ){
		
		// vars
		var type = field.get('setting') || '';
		var name = field.get('name') || '';
		var mid = modelId( type + ' ' + name );
		var model = pdc.models[ mid ] || null;
	...
	*/
	...
	pdc.getFieldSetting = function( field ) {
		
		// allow jQuery
		if( field instanceof jQuery ) {
			field = pdc.getField(field);
		}
	...
	*/
	...
	var settingsManager = new pdc.Model({
		actions: {
			'new_field': 'onNewField'
		},
		onNewField: function( field ){
			field.setting = pdc.newFieldSetting( field );
		}
	});
	...
	*/
	...
	pdc.FieldSetting = pdc.Model.extend({
		...
		setup: function( field ){
			...
			this.$el = $field;
			this.field = field;
			this.$fieldObject = $field.closest('.pdc-field-object');
			this.fieldObject = pdc.getFieldObject( this.$fieldObject );
	...
	*/
	...
	var DisplayFormatFieldSetting = pdc.FieldSetting.extend({
		type: '',
		name: '',
		render: function(){
	...
	pdc.registerFieldSetting( DatePickerDisplayFormatFieldSetting );
	pdc.registerFieldSetting( DatePickerReturnFormatFieldSetting );
	...
	*/
	...
	pdc.registerFieldSetting( DateTimePickerDisplayFormatFieldSetting );
	pdc.registerFieldSetting( DateTimePickerReturnFormatFieldSetting );
	...
	*/
	...
	pdc.registerFieldSetting( TimePickerDisplayFormatFieldSetting );
	pdc.registerFieldSetting( TimePickerReturnFormatFieldSetting );
...
(function($, undefined){
	...
	*/
	...
	var ConditionalLogicFieldSetting = pdc.FieldSetting.extend({
		type: '',
		...
		renderField: function(){
			...
			if( !choices.length ) {
				choices.push({
					id: '',
					text: pdc.__('No toggle fields available'),
				});
			}
		...
		renderOperator: function(){
			...
			// get selected field
			var $field = pdc.findFieldObject( this.ruleData('field') );
			var field = pdc.getFieldObject( $field );
			...
			conditionTypes.map(function( model ){
				choices.push({
					...
					text:	pdc.strEscape(model.prototype.label)
				});
			});
		...
		renderValue: function(){
			...
			if( choices instanceof Array ) {
				...
				pdc.renderSelect( $newSelect, choices );
		...
		addGroup: function(){
			...
			// duplicate
			var $group2 = pdc.duplicate( $group );
			
			// update h4
			$group2.find('h4').text( pdc.__('or') );
		...
		onClickAdd: function( e, $el ){
			...
			// duplciate
			var $rule = pdc.duplicate( $el.closest('.rule') );
	...
	pdc.registerFieldSetting( ConditionalLogicFieldSetting );
	...
	*/
	...
	var conditionalLogicHelper = new pdc.Model({
		actions: {
...
(function($, undefined){
	...
	*/
	...
	pdc.findFieldObject = function( key ){
		return pdc.findFieldObjects({
			key: key,
			limit: 1
		});
	};
	...
	*/
	...
	pdc.findFieldObjects = function( args ){
		
		// vars
		args = args || {};
		var selector = '.pdc-field-object';
		var $fields = false;
	...
	*/
	...
	pdc.getFieldObject = function( $field ){
		
		// allow key
		if( typeof $field === 'string' ) {
			$field = pdc.findFieldObject( $field );
		}
		
		// instantiate
		var field = $field.data('pdc');
		if( !field ) {
			field = pdc.newFieldObject( $field );
		}
	...
	*/
	...
	pdc.getFieldObjects = function( args ){
		
		// query
		var $fields = pdc.findFieldObjects( args );
		
		// loop
		var fields = [];
		$fields.each(function(){
			var field = pdc.getFieldObject( $(this) );
			fields.push( field );
		});
	...
	*/
	...
	pdc.newFieldObject = function( $field ){
		
		// instantiate
		var field = new pdc.FieldObject( $field );
		
		// action
		pdc.doAction('new_field_object', field);
	...
	*/
	...
	var eventManager = new pdc.Model({
		...
		addFieldActions: function( action ){
			...
			var callback = function( $el /*, arg1, arg2, etc*/ ){
				...
				// vars
				var fieldObjects = pdc.getFieldObjects({ parent: $el });
			...
			// add actions
			pdc.addAction(action, callback, 5);
			pdc.addAction(pluralAction, pluralCallback, 5);
			pdc.addAction(singleAction, singleCallback, 5);
	...
	*/
	...
	var fieldManager = new pdc.Model({
		
		id: 'fieldManager',
		
		events: {
			'submit #post':					'onSubmit',
			'mouseenter .pdc-field-list': 	'onHoverSortable',
		...
		onHoverSortable: function( e, $el ){
			...
			// sortable
			$el.sortable({
				handle: '.pdc-sortable-handle',
				connectWith: '.pdc-field-list',
				start: function( e, ui ){
					var field = pdc.getFieldObject( ui.item );
			        ui.placeholder.height( ui.item.height() );
			        pdc.doAction('sortstart_field_object', field, $el);
			    },
				update: function( e, ui ){
					var field = pdc.getFieldObject( ui.item );
					pdc.doAction('sortstop_field_object', field, $el);
				}
			});
		},
		...
		onDuplicateField: function( field, newField ){
			...
			if( children.length ) {
				...
				// action
				pdc.doAction('duplicate_field_objects', children, newField, field);
			}
		...
		onClickAdd: function( e, $el ){
			var $list = $el.closest('.pdc-tfoot').siblings('.pdc-field-list');
			this.addField( $list );
		},
		...
		addField: function( $list ){
			...
			// action
			pdc.doAction('add_field_object', newField);
			pdc.doAction('append_field_object', newField);
		}
	});
...
(function($, undefined){
	...
	*/
	...
	var locationManager = new pdc.Model({
		...
		initialize: function(){
			this.$el = $('#pdc-field-group-locations');
		},
		...
		changeRule: function( $rule ){
			...
			// ajaxdata
			var ajaxdata = {};
			ajaxdata.action = 'pdc/field_group/render_location_rule';
			ajaxdata.rule = pdc.serialize( $rule, prefix );
			ajaxdata.rule.id = $rule.data('id');
			ajaxdata.rule.group = $group.data('id');
			
			// temp disable
			pdc.disable( $rule.find('td.value') );
...
(function($, undefined){
	...
	var _pdc = pdc.getCompatibility( pdc );
	
	/**
	*  fieldGroupCompatibility
	*
	*  Compatibility layer for extinct pdc.field_group 
	*
	*  @date	15/12/17
	*  @since	5.7.0
	*
	...
	*/
	...
	_pdc.field_group = {
		...
		save_field: function( $field, type ){
			...
			pdc.getFieldObject( $field ).save( type );
		},
		
		delete_field: function( $field, animate ){
			animate = (animate !== undefined) ? animate : true;
			pdc.getFieldObject( $field ).delete({
				animate: animate
			});
		},
		
		update_field_meta: function( $field, name, value ){
			pdc.getFieldObject( $field ).prop( name, value );
		},
		
		delete_field_meta: function( $field, name ){
			pdc.getFieldObject( $field ).prop( name, null );
		}
	};
	...
	*
	*  Compatibility layer for extinct pdc.field_group.field_object
	*
	*  @date	15/12/17
	*  @since	5.7.0
	*
	...
	*/
	...
	_pdc.field_group.field_object = pdc.model.extend({
		...
		selector: function(){
			...
			if( type ) {
				selector += '-' + type;
				selector = pdc.str_replace('_', '-', selector);
			}
		...
		_add_action: function( name, callback ) {
			...
			// add action
			pdc.add_action( this.tag(name), function( $field ){
		...
		_add_filter: function( name, callback ) {
			...
			// add action
			pdc.add_filter( this.tag(name), function( $field ){
		...
		setting: function( name ) {
			...
			return this.$settings.find('> .pdc-field-setting-' + name);
	...
	*/
	...
	var actionManager = new pdc.Model({
		...
		onOpenFieldObject: function( field ){
			pdc.doAction('open_field', field.$el);
			pdc.doAction('open_field/type=' + field.get('type'), field.$el);
			
			pdc.doAction('render_field_settings', field.$el);
			pdc.doAction('render_field_settings/type=' + field.get('type'), field.$el);
		},
		
		onCloseFieldObject: function( field ){
			pdc.doAction('close_field', field.$el);
			pdc.doAction('close_field/type=' + field.get('type'), field.$el);
		},
		
		onAddFieldObject: function( field ){
			pdc.doAction('add_field', field.$el);
			pdc.doAction('add_field/type=' + field.get('type'), field.$el);
		},
		
		onDuplicateFieldObject: function( field ){
			pdc.doAction('duplicate_field', field.$el);
			pdc.doAction('duplicate_field/type=' + field.get('type'), field.$el);
		},
		
		onDeleteFieldObject: function( field ){
			pdc.doAction('delete_field', field.$el);
			pdc.doAction('delete_field/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectType: function( field ){
			pdc.doAction('change_field_type', field.$el);
			pdc.doAction('change_field_type/type=' + field.get('type'), field.$el);
		...
		onChangeFieldObjectLabel: function( field ){
			pdc.doAction('change_field_label', field.$el);
			pdc.doAction('change_field_label/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectName: function( field ){
			pdc.doAction('change_field_name', field.$el);
			pdc.doAction('change_field_name/type=' + field.get('type'), field.$el);
		},
		
		onChangeFieldObjectParent: function( field ){
			pdc.doAction('update_field_parent', field.$el);
		}
	});
