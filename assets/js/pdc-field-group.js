(function($){
	...
	pdc.field_group = pdc.model.extend({
		...
		events: {
			...
			'mouseenter .pdc-field-list': 	'sortable'
		},
		...
		*/
		...
		init: function(){
			...
			// $el
			this.$fields = $('#pdc-field-group-fields');
			this.$locations = $('#pdc-field-group-locations');
			this.$options = $('#pdc-field-group-options');
			
			
			// disable validation
			pdc.validation.active = 0;
		...
		*/
		...
		render_fields: function(){
			...
			$('.pdc-field-list').each(function(){
				
				// vars
				var $fields = $(this).children('.pdc-field-object');
				...
				$fields.each(function( i ){
					...
					// update icon number
					$(this).children('.handle').find('.pdc-icon').html( i+1 );
		...
		*/
		...
		update_field_meta: function( $el, name, value ){
			...
			if( !$input.exists() ) {
				...
				// replcae
				html = pdc.str_replace('ID', name, html);
		...
		*/
		...
		save_field: function( $el, type ){
			...
			pdc.do_action('save_field', $el, type);
		...
		*/
		...
		submit: function( e ){
			...
			if( !$title.val() ) {
				...
				// unlock form
				pdc.validation.toggle( e.$el, 'unlock' );
				
				
				// alert
				alert( pdc._e('title_is_required') );
			...
			$('.pdc-field-object').each(function(){
				...
				if( save == 'settings' ) {
					...
					$(this).children('.settings').find('[name^="pdc_fields[' + ID + ']"]').remove();
					
				} else {
					
					$(this).find('[name^="pdc_fields[' + ID + ']"]').remove();
		...
		*/
		...
		trash: function( e ){
			...
			var result = confirm( pdc._e('move_to_trash') );
		...
		*/
		...
		render_field: function( $el ){
			...
			// update required
			$handle.find('.li-field-label .pdc-required').remove();
			
			if( required ) {
				
				$handle.find('.li-field-label strong').append('<span class="pdc-required">*</span>');
			...
			// action for 3rd party customization
			pdc.do_action('render_field_handle', $el, $handle);
		...
		*/
		...
		move_field: function( $field ){
			...
			if( !ajax_data.field_id ) {
				...
				$field.find('.pdc-field-object').each(function(){
			...
			if( warning ) {
				...
				alert( pdc._e('move_field_warning') );
				return;
		...
		*/
		...
		move_field_confirm: function( $field, html ){
			...
			// submit form
			$('#pdc-move-field-form').on('submit', function(){
		...
		*/
		...
		remove_field: function( $el ){
			...
			// vars
			var $field_list	= $el.closest('.pdc-field-list');
			...
			if( !$field_list.children('.pdc-field-object').length ) {
		...
		*/
		...
		change_field_type: function( $select ){
			...
			var $tbody		= $select.closest('tbody'),
				$el			= $tbody.closest('.pdc-field-object'),
				$parent		= $el.parent().closest('.pdc-field-object'),
			...
			// update class
			$el.removeClass( 'pdc-field-object-' + pdc.str_replace('_', '-', old_type) );
			$el.addClass( 'pdc-field-object-' + pdc.str_replace('_', '-', new_type) );
			...
			// get settings
			var $settings = $tbody.children('.pdc-field[data-setting="' + old_type + '"]'),
				html = '';
			...
			pdc.update( key + '_settings_' + old_type, html );
			...
			html = pdc.get( key + '_settings_' + new_type );
			
			if( html ) {
				
				// append settings
				$tbody.children('.pdc-field[data-name="conditional_logic"]').before( html );
				
				
				// remove field settings html
				pdc.update( key + '_settings_' + new_type, '' );
				
				
				// trigger event
				pdc.do_action('change_field_type', $el);
			...
			// add loading
			var $tr = $('<tr class="pdc-field"><td class="pdc-label"></td><td class="pdc-input"><div class="pdc-loading"></div></td></tr>');
			
			
			// add $tr
			$tbody.children('.pdc-field[data-name="conditional_logic"]').before( $tr );
			
			
			var ajax_data = {
				action		: 'pdc/field_group/render_field_settings',
				nonce		: pdc.o.nonce,
				parent		: pdc.o.post_id,
				field_group	: pdc.o.post_id,
		...
		*/
		...
		change_field_label: function( $el ) {
			...
			// action for 3rd party customization
			pdc.do_action('change_field_label', $el);
		...
		*/
		...
		change_field_name: function( $el ) {
			...
			// action for 3rd party customization
			pdc.do_action('change_field_name', $el);
	...
	*/
	...
	pdc.field_group.field = pdc.model.extend({
		...
		events: {
			...
			'change .pdc-field-object input':		'save',
			'change .pdc-field-object textarea':	'save',
			'change .pdc-field-object select':		'save'
		},
		...
		*/
		...
		edit: function( e ){
			...
			pdc.field_group.edit_field( e.$field );
				
		},
		
		duplicate: function( e ){
			
			pdc.field_group.duplicate_field( e.$field );
				
		},
		
		move: function( e ){
			
			pdc.field_group.move_field( e.$field );
				
		},
		
		delete: function( e ){
			
			pdc.field_group.delete_field( e.$field );
				
		},
		
		add: function( e ){
			
			var $list = e.$el.closest('.pdc-field-list-wrap').children('.pdc-field-list');
			
			pdc.field_group.add_field( $list );
				
		},
		
		change_type: function( e ){
			
			pdc.field_group.change_field_type( e.$el );
			
		},
		
		change_label: function( e ){
			
			pdc.field_group.change_field_label( e.$field );
			
		},
		
		change_name: function( e ){
			
			pdc.field_group.change_field_name( e.$field );
			
		},
		
		render: function( e ){
			
			pdc.field_group.render_field( e.$field );
				
		},
		
		save: function( e ){
			
			pdc.field_group.save_field( e.$field );
	...
	*/
	...
	pdc.field_group.conditional_logic = pdc.model.extend({
		...
		*/
		...
		render_fields: function(){
			...
			$('.pdc-field-object.open').each(function(){
		...
		*/
		...
		render_field: function( $field ){
			...
			var key			= $field.attr('data-key'),
				$ancestors	= $field.parents('.pdc-field-list'),
				$tr			= $field.find('.pdc-field[data-name="conditional_logic"]:last');
			...
			$.each( $ancestors, function( i ){
				...
				// vars
				var group = (i == 0) ? pdc._e('sibling_fields') : pdc._e('parent_fields');
				
				
				// loop over fields
				$(this).children('.pdc-field-object').each(function(){
		...
		*/
		...
		render_rule: function( $tr, triggers ) {
			...
			if( triggers ) {
				...
				pdc.render_select( $trigger, triggers );
				
			}
			
			
			// vars
			var $field		= $('.pdc-field-object[data-key="' + $trigger.val() + '"]'),
			...
			// select				
				...
				// vars
				var lines = $field.find('.pdc-field[data-name="choices"] textarea').val().split("\n");	
				...
				// allow null
				var $allow_null = $field.find('.pdc-field[data-name="allow_null"]');
			...
			// update select
			pdc.render_select( $value, choices );
	...
	*/
	...
	pdc.field_group.locations = pdc.model.extend({
		...
		*/
		...
		change_rule: function( e ){
			...
			// add loading gif
			var $div = $('<div class="pdc-loading"></div>');
			...
			$.ajax({
				...
				data: pdc.prepare_for_ajax({
					'action':	'pdc/field_group/render_location_value',
					'rule_id':	rule_id,
					'group_id':	group_id,
					'param':	$select.val(),
					'value':	''
	...
	*/
	...
	pdc.field_group.field_object = pdc.model.extend({
		...
		_add_event: function( name, callback ) {
			...
			var model = this,
				...
				context = pdc.field_group.get_selector(model.type);
	...
	*/
	...
	pdc.field_group.field_objects = pdc.model.extend({
		...
		_save_field: function( $el ){
			...
			pdc.do_action('save_field/type=' + $el.data('type'), $el);
			
		},
		
		_open_field: function( $el ){
			
			pdc.do_action('open_field/type=' + $el.data('type'), $el);
			pdc.do_action('render_field_settings', $el);
			
		},
		
		_close_field: function( $el ){
			
			pdc.do_action('close_field/type=' + $el.data('type'), $el);
			
		},
		
		_wipe_field: function( $el ){
			
			pdc.do_action('wipe_field/type=' + $el.data('type'), $el);
			
		},
		
		_add_field: function( $el ){
			
			pdc.do_action('add_field/type=' + $el.data('type'), $el);
			
		},
		
		_duplicate_field: function( $el ){
			
			pdc.do_action('duplicate_field/type=' + $el.data('type'), $el);
			
		},
		
		_delete_field: function( $el ){
			
			pdc.do_action('delete_field/type=' + $el.data('type'), $el);
			
		},
		
		_change_field_type: function( $el ){
			
			pdc.do_action('change_field_type/type=' + $el.data('type'), $el);
			pdc.do_action('render_field_settings', $el);
		},
		
		_change_field_label: function( $el ){
			
			pdc.do_action('change_field_label/type=' + $el.data('type'), $el);
			
		},
		
		_change_field_name: function( $el ){
			
			pdc.do_action('change_field_name/type=' + $el.data('type'), $el);
			
		},
		
		_render_field_settings: function( $el ){
			
			pdc.do_action('render_field_settings/type=' + $el.data('type'), $el);
	...
	*/
	...
	pdc.field_group.append = pdc.model.extend({
		...
		render: function( $el ){
			...
			// vars
			var $wrap = $sibling.children('.pdc-input'),
				$ul = $wrap.children('.pdc-hl');
			
			
			// append ul if doesn't exist
			if( !$ul.exists() ) {
				
				$wrap.wrapInner('<ul class="pdc-hl"><li></li></ul>');
			...
			// create $li
			var $li = $('<li></li>').append( $el.children('.pdc-input').children() );
		...
		_render_field_settings: function( $el ){
			...
			// loop
			$el.find('.pdc-field[data-append]').each(function(){
	...
	*/
	...
	var pdc_settings_select = pdc.field_group.field_object.extend({
		...
		events: {
			'change .pdc-field-setting-ui input': 'render'
		},
	...
	*/
	...
	var pdc_settings_radio = pdc.field_group.field_object.extend({
		...
		events: {
			'change .pdc-field-setting-other_choice input': 'render'
		},
	...
	*/
	...
	var pdc_settings_true_false = pdc.field_group.field_object.extend({
	...
	*/
	...
	var pdc_settings_date_picker = pdc.field_group.field_object.extend({
		...
		events: {
			'change .pdc-field-setting-display_format input':	'render',
			'change .pdc-field-setting-return_format input':	'render'
		},
	...
	*/
	...
	var pdc_settings_date_time_picker = pdc_settings_date_picker.extend({
	...
	*/
	...
	var pdc_settings_tab = pdc.field_group.field_object.extend({
	...
	*/
	...
	var pdc_settings_message = pdc_settings_tab.extend({
	...
	*/
	...
	pdc.field_group.screen = pdc.model.extend({
		...
		events: {
			'click #pdc-field-key-hide': 'toggle'
		},
		...
		ready: function(){
			...
			// vars
			var $el = $('#adv-settings'),
				$append = $el.find('#pdc-append-show-on-screen');
		...
		toggle: function( e ){
			...
			// update user setting
			pdc.update_user_setting('show_field_keys', val);
		...
		render: function(){
			...
			// vars
			var options = pdc.serialize_form( $('#adv-settings') );
			...
			// toggle class
			var $fields = pdc.field_group.$fields;
	...
	*/
	...
	pdc.field_group.sub_fields = pdc.model.extend({
    	...
    	*/
    	...
    	update_field_parent: function( $el ){
	    	...
	    	if( !$el.hasClass('pdc-field-object') ) return;
	    	
	    	
	    	// vars
	    	var $parent = $el.parent().closest('.pdc-field-object'),
		    	val = pdc.get('post_id');
			...
			if( $parent.exists() ) {
				...
				// set as parent ID
				val = pdc.field_group.get_field_meta( $parent, 'ID' );
				
				
				// if parent is new, no ID exists
				if( !val ) {
					
					val = pdc.field_group.get_field_meta( $parent, 'key' );
			...
			// update parent
			pdc.field_group.update_field_meta( $el, 'parent', val );
	    	
	    	
	    	// action for 3rd party customization
			pdc.do_action('update_field_parent', $el, $parent);
    	...
    	*/
    	...
    	duplicate_field: function( $el ) {
	    	...
	    	// vars
			var $fields = $el.find('.pdc-field-object');
	    	...
	    	$fields.each(function(){
		    	...
		    	// vars
		    	var $parent = $(this).parent().closest('.pdc-field-object'),
		    		key = pdc.field_group.get_field_meta( $parent, 'key');
		    		
		    	
		    	// wipe field
		    	pdc.field_group.wipe_field( $(this) );
		    	
		    	
		    	// update parent
		    	pdc.field_group.update_field_meta( $(this), 'parent', key );
		    	
		    	
		    	// save field
		    	pdc.field_group.save_field( $(this) );
