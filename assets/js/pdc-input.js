...
(function($){
	...
	*/
	...
	pdc = {
		...
		*  update
		*
		*  This function will update a value found in pdc.o
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*  get
		*
		*  This function will return a value found in pdc.o
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*  _e
		*
		*  This functiln will return a string found in pdc.l10n
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
...
*/
		...
		*/
		...
		do_action: function() { //console.log('pdc.do_action(%o)', arguments);
		...
		*/
		...
		apply_filters: function() { //console.log('pdc.apply_filters(%o)', arguments);
		...
		*/
		...
		get_fields: function( s, $el, all ){
			...
			// debug
			//console.log( 'pdc.get_fields(%o, %o, %o)', args, $el, all );
			//console.time("pdc.get_fields");
			...
			if( !all ) {
				...
				// filter
				$fields = pdc.apply_filters('get_fields', $fields);
								
			}
			
			
			//console.log('get_fields(%o, %o, %o) %o', s, $el, all, $fields);
			//console.log('pdc.get_fields(%o):', this.get_selector(s) );
			//console.timeEnd("pdc.get_fields");
		...
		*/
		...
		disable: function( $input, context ){
			...
			// bail early if is .pdc-disabled
			if( $input.hasClass('pdc-disabled') ) return false;
			...
			if( context ) {
				...
				// vars
				var disabled = $input.data('pdc_disabled') || [],
				...
				if( i < 0 ) {
					...
					// update
					$input.data('pdc_disabled', disabled);
		...
		*/
		...
		remove_el : function( $el, callback, end_height ){
			...
			// wrap field
			$el.wrap( '<div class="pdc-temp-wrap" style="height:' + $el.outerHeight(true) + 'px"></div>' );
			...
			// remove
			$el.parent('.pdc-temp-wrap').animate({ height : end_height }, 250, function(){
		...
		*/
		...
		open_popup : function( args ){
			...
			var tmpl = [
				'<div id="pdc-popup">',
					'<div class="pdc-popup-box pdc-box">',
						'<div class="title"><h3></h3><a href="#" class="pdc-icon -cancel grey pdc-close-popup"></a></div>',
						'<div class="inner"></div>',
			...
			$('#pdc-popup').on('click', '.bg, .pdc-close-popup', function( e ){
		...
		*/
		...
		update_user_setting : function( name, value ) {
			...
			$.ajax({
				...
				dataType	: 'html',
				type		: 'post',
				data		: pdc.prepare_for_ajax({
					'action'	: 'pdc/update_user_setting',
					'name'		: name,
					'value'		: value
				})
			});
		...
		*/
		...
		prepare_for_ajax : function( args ) {
			...
			// vars
			args.nonce = pdc.get('nonce');
			args.post_id = pdc.get('post_id');
			
			
			// filter for 3rd party customization
			args = pdc.apply_filters('prepare_for_ajax', args);	
		...
		*/
		...
		get_ajax_message: function( json ) {
			...
			// PHP error (too may themes will have warnings / errors. Don't show these in pdc taxonomy popup)
/*
...
*/
		...
		*/
		...
		duplicate: function( args ){
			...
			// replace
			if( !args.replace ) args.replace = pdc.get_uniqid();
			...
			pdc.do_action('before_duplicate', $el);
			...
			// remove pdc-clone (may be a clone)
			$el2.removeClass('pdc-clone');
			
			
			// remove JS functionality
			pdc.do_action('remove', $el2);
			...
			// after
			// - allow pdc to modify DOM
			pdc.do_action('after_duplicate', $el, $el2 );
	...
	pdc.field = pdc.model.extend({
		...
		_add_event: function( name, callback ) {
			...
			$(document).on(event, context + ' ' + selector, function( e ){
				...
				// append $el to event object
				e.$el = $(this);
				e.$field = pdc.get_closest_field(e.$el, model.type);
	...
	*/
	...
	pdc.fields = pdc.model.extend({
		...
		_prepare: function( $el ){
			...
			pdc.get_fields('', $el).each(function(){
				
				pdc.do_action('prepare_field', $(this));
		...
		_prepare_field: function( $el ){
			...
			pdc.do_action('prepare_field/type=' + $el.data('type'), $el);
		...
		_ready: function( $el ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('ready_field', $(this));
		...
		_ready_field: function( $el ){
			...
			pdc.do_action('ready_field/type=' + $el.data('type'), $el);
		...
		_append: function( $el ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('append_field', $(this));
		...
		_append_field: function( $el ){
			...
			pdc.do_action('append_field/type=' + $el.data('type'), $el);
		...
		_load_field: function( $el ){
			...
			pdc.do_action('load_field/type=' + $el.data('type'), $el);
		...
		_remove: function( $el ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('remove_field', $(this));
		...
		_remove_field: function( $el ){
			...
			pdc.do_action('remove_field/type=' + $el.data('type'), $el);
		...
		_sortstart: function( $el, $placeholder ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('sortstart_field', $(this), $placeholder);
		...
		_sortstart_field: function( $el, $placeholder ){
			...
			pdc.do_action('sortstart_field/type=' + $el.data('type'), $el, $placeholder);
		...
		_sortstop: function( $el, $placeholder ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('sortstop_field', $(this), $placeholder);
		...
		_sortstop_field: function( $el, $placeholder ){
			...
			pdc.do_action('sortstop_field/type=' + $el.data('type'), $el, $placeholder);
		...
		_hide: function( $el, context ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('hide_field', $(this), context);
		...
		_hide_field: function( $el, context ){
			...
			pdc.do_action('hide_field/type=' + $el.data('type'), $el, context);
		...
		_show: function( $el, context ){
			...
			pdc.get_fields('', $el).each(function(){
				...
				pdc.do_action('show_field', $(this), context);
		...
		_show_field: function( $el, context ){
			...
			pdc.do_action('show_field/type=' + $el.data('type'), $el, context);
	...
	*/
	...
	pdc.layout = pdc.model.extend({
		...
		render_tables: function( $el ){ 
			...
			// vars
			var $tables = $el.find('.pdc-table:visible');
		...
		render_table: function( $table ){
			...
			// vars
			var $ths = $table.find('> thead th.pdc-th'),
				colspan = 1,
				available_width = 100;
			...
			var $trs = $table.find('> tbody > tr'),
				$tds = $trs.find('> td.pdc-field');
			
			
			// remove clones if has visible rows
			if( $trs.hasClass('pdc-clone') && $trs.length > 1 ) {
				
				$tds = $trs.not('.pdc-clone').find('> td.pdc-field');
			...
			// update colspan
			$table.find('.pdc-row .pdc-field.-collapsed-target').removeAttr('colspan');
			$table.find('.pdc-row.-collapsed .pdc-field.-collapsed-target').attr('colspan', colspan);
		...
		render_groups: function( $el ){
			...
			// vars
			var $groups = $el.find('.pdc-fields:visible');
			
			
			// appent self if is '.pdc-fields'
			if( $el && $el.is('.pdc-fields') ) {
		...
		render_group: function( $el ){
			...
			// reset fields
			$fields.removeClass('pdc-r0 pdc-c0').css({'min-height': 0});
	...
	*/
	...
	$(document).on('change', '.pdc-field input, .pdc-field textarea, .pdc-field select', function(){
		
		// preview hack
		if( $('#pdc-form-data input[name="_pdcchanged"]').exists() ) {
		
			$('#pdc-form-data input[name="_pdcchanged"]').val(1);
			
		}
		
		
		// action for 3rd party customization
		pdc.do_action('change', $(this));
	...
	*/
	...
	$(document).on('click', '.pdc-field a[href="#"]', function( e ){
	...
	*/
	...
	pdc.tooltip = pdc.model.extend({
		
		$el: null,
		
		events: {
			'mouseenter .pdc-js-tooltip':	'on',
			'mouseleave .pdc-js-tooltip':	'off'
		},
		...
		on: function( e ){
			...
			// $t
			this.$el = $('<div class="pdc-tooltip">' + title + '</div>');
	...
	pdc.postbox = pdc.model.extend({
		
		events: {
			'mouseenter .pdc-postbox .handlediv':	'on',
			'mouseleave .pdc-postbox .handlediv':	'off'
		},
		...
		render: function( args ){
			...
			// .inside class
			$postbox.children('.inside').addClass('pdc-fields').addClass('-' + args.label);
			...
			if( args.edit_url ) {
				...
				$postbox.children('.hndle').append('<a href="' + args.edit_url + '" class="dashicons dashicons-admin-generic pdc-hndle-cog pdc-js-tooltip" title="' + args.edit_title + '"></a>');
	...
	*/
	...
	pdc.add_action('sortstart', function( $item, $placeholder ){
	...
	*/
	...
	pdc.add_action('before_duplicate', function( $orig ){
	...
	pdc.add_action('after_duplicate', function( $orig, $duplicate ){
...
/*
	...
	console.time("pdc_test_ready");
	console.time("pdc_test_load");
	
	pdc.add_action('ready', function(){
		
		console.timeEnd("pdc_test_ready");
		
	}, 999);
	
	pdc.add_action('load', function(){
		
		console.timeEnd("pdc_test_load");
...
(function($){
	...
	pdc.ajax = pdc.model.extend({
		...
		events: {
			...
			'change .pdc-taxonomy-field[data-save="1"] input':		'_change_term',
			'change .pdc-taxonomy-field[data-save="1"] select':		'_change_term'
		},
		o: {
			//'post_id':		0,
			//'page_template':	0,
		...
		ready: function(){
			...
			// update post_id
			this.update('post_id', pdc.get('post_id'));
...
*/
		...
		fetch: function(){
			...
			// add action url
			data.action = 'pdc/post/get_field_groups';
			...
			this.xhr = $.ajax({
				url:		pdc.get('ajaxurl'),
				data:		pdc.prepare_for_ajax( data ),
				type:		'post',
				dataType:	'json',
				
				success: function( json ){
					
					if( pdc.is_ajax_success( json ) ) {
		...
		render: function( json ){
			...
			$.each(json, function( k, field_group ){
				...
				// vars
				var $postbox = $('#pdc-' + field_group.key),
					$toggle = $('#pdc-' + field_group.key + '-hide'),
					$label = $toggle.parent();
				...
				if( k === 0 ) {
					...
					$('#pdc-style').html( field_group.style );
					
				}
				
				
				// enable inputs
				$postbox.find('.pdc-hidden-by-postbox').prop('disabled', false);
				
			});
			
			
			// disable inputs
			$('.pdc-postbox.pdc-hidden').find('select, textarea, input').not(':disabled').each(function(){
				
				$(this).addClass('pdc-hidden-by-postbox').prop('disabled', true);
		...
		sync_taxonomy_terms: function(){
			...
			$('.categorychecklist, .pdc-taxonomy-field').each(function(){
				...
				if( $el.is('.pdc-taxonomy-field') && $el.attr('data-save') != '1' ) {
...
(function($){
	...
	pdc.fields.checkbox = pdc.field.extend({
		
		type: 'checkbox',
		
		events: {
			'change input':				'_change',
			'click .pdc-add-checkbox':	'_add'
		},
		...
		*/
		...
		add: function(){
			...
			// vars
			var html = '<li><input class="pdc-checkbox-custom" type="checkbox" checked="checked" /><input type="text" name="'+name+'" /></li>';
			
			
			// append
			this.$ul.find('.pdc-add-checkbox').parent('li').before( html );	
		...
		_change: function( e ){
			...
			// vars
			var $ul = this.$ul,
				$inputs = $ul.find('input[type="checkbox"]').not('.pdc-checkbox-toggle'),
				checked = e.$el.is(':checked');
			
			
			// is toggle?
			if( e.$el.hasClass('pdc-checkbox-toggle') ) {
			...
			// is custom
			if( e.$el.hasClass('pdc-checkbox-custom') ) {
			...
			if( !$ul.find('.pdc-checkbox-toggle').exists() ) {
			...
			// update toggle
			$ul.find('.pdc-checkbox-toggle').prop('checked', checked);
...
(function($){
	...
	pdc.fields.color_picker = pdc.field.extend({
		...
		initialize: function(){
			...
			var args = {
 			...
 			// filter
 			var args = pdc.apply_filters('color_picker_args', args, this.$field);
...
(function($){
	...
	pdc.conditional_logic = pdc.model.extend({
		...
		events: {
			'change .pdc-field input': 		'change',
			'change .pdc-field textarea': 	'change',
			'change .pdc-field select': 	'change'
		},
		...
		*/
		...
		render: function( $el ){
			...
			// get targets
			var $targets = pdc.get_fields( '', $el, true );
		...
		*/
		...
		change: function( e ){
			...
			// vars
			var $input = e.$el,
				$field = pdc.get_field_wrap( $input ),
				key = $field.data('key');
			...
			for( var i in this.triggers[ key ] ) {
				...
				// get targets
				var $targets = pdc.get_fields(target_key, $parent, true);
		...
		*/
		...
		show_field: function( $field ){
			...
			// enable
			pdc.enable_form( $field, 'condition_'+key );
			
						
			// action for 3rd party customization
			pdc.do_action('show_field', $field, 'conditional_logic' );
		...
		*/
		...
		hide_field : function( $field ){
			...
			// disable
			pdc.disable_form( $field, 'condition_'+key );
						
			
			// action for 3rd party customization
			pdc.do_action('hide_field', $field, 'conditional_logic' );
		...
		*/
		...
		get_trigger: function( $target, key ){
			...
			// vars
			var selector = pdc.get_selector( key );
...
(function($){
	...
	*/
	...
	pdc.datepicker = pdc.model.extend({
	...
	pdc.fields.date_picker = pdc.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-date-picker');
			this.$input = this.$el.find('input[type="text"]');
			this.$hidden = this.$el.find('input[type="hidden"]');
			
			
			// get options
			this.o = pdc.get_data( this.$el );
		...
		initialize: function(){
			...
			// filter for 3rd party customization
			args = pdc.apply_filters('date_picker_args', args, this.$field);
			
			
			// add date picker
			pdc.datepicker.init( this.$input, args );
			
			
			// action for 3rd party customization
			pdc.do_action('date_picker_init', this.$input, args, this.$field);
...
(function($){
	...
	*/
	...
	pdc.datetimepicker = pdc.model.extend({
	...
	pdc.fields.date_time_picker = pdc.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-date-time-picker');
			this.$input = this.$el.find('input[type="text"]');
			this.$hidden = this.$el.find('input[type="hidden"]');
		...
		initialize: function(){
			...
			// filter for 3rd party customization
			args = pdc.apply_filters('date_time_picker_args', args, this.$field);
			
			
			// add date time picker
			pdc.datetimepicker.init( this.$input, args );
			
			
			// action for 3rd party customization
			pdc.do_action('date_time_picker_init', this.$input, args, this.$field);
...
(function($){
	...
	pdc.fields.file = pdc.field.extend({
		...
		*/
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-file-uploader');
		...
		*/
		...
		add: function() {
			...
			// get repeater
			var $repeater = pdc.get_closest_field( $field, 'repeater' );
			
			
			// popup
			var frame = pdc.media.popup({
				
				title:		pdc._e('file', 'select'),
				mode:		'select',
				type:		'',
				...
				select: function( attachment, i ) {
			    	...
			    	if( i > 0 ) {
						...
						$tr.nextAll('.pdc-row:visible').each(function(){
							
							// get next $field
							$field = pdc.get_field( key, $(this) );
							...
							if( $field.find('.pdc-file-uploader.has-value').exists() ) {
						...
						if( !$field ) {
							...
							$tr = pdc.fields.repeater.doFocus( $repeater ).add();
		...
		*/
		...
		edit: function() {
			...
			var frame = pdc.media.popup({
				...
				title:		pdc._e('file', 'edit'),
				button:		pdc._e('file', 'update'),
				mode:		'edit',
				attachment:	val,
...
(function($){
	...
	pdc.fields.google_map = pdc.field.extend({
		...
		events: {
			...
			'blur .search': 					'_blur',
			//'paste .search': 					'_paste',
			'mousedown .pdc-google-map':		'_mousedown'
		},
		
		focus: function(){
			
			// get elements
			this.$el = this.$field.find('.pdc-google-map');
		...
		*/
		...
		is_ready: function(){ 
			...
			// check exists (ok)
			if( pdc.isset(window, 'google', 'maps') ) {
			...
			if( this.url ) {
				...
				// enqueue
				pdc.enqueue_script(this.url, function(){
		...
		*/
		...
		initialize: function(){
			...
			// map
			var map_args = pdc.apply_filters('google_map_args', {
			...
			// marker
			var marker_args = pdc.apply_filters('google_map_marker_args', {
			...
			// action for 3rd party customization
			pdc.do_action('google_map_init', this.map, this.map.marker, this.$field);
		...
		update: function( lat, lng ){
		    ...
		    // update inputs
		    pdc.val( this.$el.find('.input-lat'), lat );
		    pdc.val( this.$el.find('.input-lng'), lng );
			...
			// action
			pdc.do_action('google_map_change', latlng, this.map, this.$field);
		...
		sync: function(){
			...
			this.geocoder.geocode({ 'latLng' : latlng }, function( results, status ){
				...
				// update input
				pdc.val( self.$el.find('.input-address'), location.formatted_address );
		...
		*/
		...
		_clear: function( e ){ // console.log('_clear');
			...
			// clear inputs
			pdc.val( this.$el.find('.input-address'), '' );
			pdc.val( this.$el.find('.input-lat'), '' );
			pdc.val( this.$el.find('.input-lng'), '' );
		...
		_locate: function( e ){ // console.log('_locate');
			...
			if( !navigator.geolocation ) {
				...
				alert( pdc._e('google_map', 'browser_support') );
				return this;
...
(function($){
	...
	pdc.fields.image = pdc.field.extend({
		...
		*/
		...
		focus: function(){
			...
			// vars
			this.$el = this.$field.find('.pdc-image-uploader');
			this.$input = this.$el.find('input[type="hidden"]');
		...
		*/
		...
		prepare: function( attachment ) {
			...
			if( attachment.id ) {
				...
				data.url = pdc.maybe_get(data, 'sizes.'+this.o.preview_size+'.url', data.url);
		...
		*/
		...
		add: function() {
			...
			// get repeater
			var $repeater = pdc.get_closest_field( this.$field, 'repeater' );
			
			
			// popup
			var frame = pdc.media.popup({
				
				title:		pdc._e('image', 'select'),
				mode:		'select',
				type:		'image',
				field:		$field.data('key'),
				multiple:	$repeater.exists(),
				...
				select: function( attachment, i ) {
			    	...
			    	if( i > 0 ) {
						...
						$tr.nextAll('.pdc-row:visible').each(function(){
							...
							// bail early if next file uploader has value
							if( $field.find('.pdc-image-uploader.has-value').exists() ) {
		...
		*/
		...
		edit: function() {
			...
			var frame = pdc.media.popup({
				...
				title:		pdc._e('image', 'edit'),
				button:		pdc._e('image', 'update'),
				mode:		'edit',
				attachment:	val,
		...
		*/
		...
		change: function( e ){
			...
			pdc.fields.file.get_file_info( e.$el, this.$input );
	...
	pdc.media = pdc.model.extend({
		...
		*/
		...
		popup: function( args ) {
			...
			// settings
			var settings = pdc.parse_args( args, {
		...
		*/
		...
		_get_select_frame_settings: function( frame, settings ){
			...
			// button
			frame._button = pdc._e('media', 'select');
		...
		_get_edit_frame_settings: function( frame, settings ){
			...
			// button
			frame._button = pdc._e('media', 'update');
...
*/
			...
			frame.on('open',function() {
				...
				// add class
				this.$el.closest('.media-modal').addClass('pdc-media-modal -' +settings.mode );
		...
		_add_select_frame_events: function( frame, settings ){
			...
			if( pdc.isset(_wpPluploadSettings, 'defaults', 'multipart_params') ) {
				...
				_wpPluploadSettings.defaults.multipart_params._pdcuploader = settings.field;
			...
			frame.on('content:activate:browse', function(){
				...
				if( settings.type == 'image' ) {
					...
					// update all
					filters.filters.all.text = pdc._e('image', 'all');
				...
				if( settings.library == 'uploadedTo' ) {
					...
					// add 'uploadedTo' text
					filters.$el.parent().append('<span class="pdc-uploadedTo">' + pdc._e('image', 'uploadedTo') + '</span>');
				...
				$.each( filters.filters, function( k, filter ){
					...
					filter.props._pdcuploader = settings.field;
					
				});
				
				
				// add _pdcuplaoder to search
				search.model.attributes._pdcuploader = settings.field;
		...
		*/
		...
		new_media_frame: function( settings ){
			...
			if( pdc.isset(Query, 'mirroring', 'args') ) {
				
				Query.mirroring.args._pdcuploader = settings.field;
		...
		ready: function(){
			...
			// vars
			var version = pdc.get('wp_version'),
				browser = pdc.get('browser'),
				post_id = pdc.get('post_id');
			
			
			// update wp.media
			if( pdc.isset(window,'wp','media','view','settings','post') && $.isNumeric(post_id) ) {
...
/*
		add_pdcEmbed: function(){
			...
			// add view
			wp.media.view.pdcEmbed = wp.media.view.Embed.extend({
				
				initialize: function() {
				
					// set attachments
					this.model.props.attributes = this.controller.pdc.attachment || {};
				...
				refresh: function() {
					...
					// vars
					var attachment = pdc.parse_args(this.model.props.attributes, {
						url: '',
						filename: '',
						title: '',
						caption: '',
...
*/
		...
		customize_Attachment: function(){
			...
			wp.media.view.Attachment.Library = AttachmentLibrary.extend({
				...
				render: function() {
					...
					// vars
					var frame = pdc.media.frame(),
						errors = pdc.maybe_get(this, 'model.attributes.pdc_errors');
				...
				*/
				...
				toggleSelection: function( options ) {
					...
					if( frame && errors ) {
						...
						// vars
						var filename = pdc.maybe_get(this, 'model.attributes.filename', '');
						...
						$sidebar.prepend([
							'<div class="pdc-selection-error">',
								'<span class="selection-error-label">' + pdc._e('restricted') +'</span>',
		...
		customize_AttachmentCompat: function(){
			...
			wp.media.view.AttachmentCompat = AttachmentCompat.extend({
				...
				add_pdc_expand_button: function(){
					...
					if( $el.find('.media-frame-router .pdc-expand-details').exists() ) return;
					
					
					// create button
					var $a = $([
						'<a href="#" class="pdc-expand-details">',
							'<span class="is-closed"><span class="pdc-icon -left small grey"></span>' + pdc._e('expand_details') +  '</span>',
							'<span class="is-open"><span class="pdc-icon -right small grey"></span>' + pdc._e('collapse_details') +  '</span>',
						'</a>'
					].join('')); 
					...
					$a.on('click', function( e ){
						...
						if( $el.hasClass('pdc-expanded') ) {
						
							$el.removeClass('pdc-expanded');
							
						} else {
							
							$el.addClass('pdc-expanded');
				...
				render: function() {
					...
					clearTimeout( pdc.media.render_timout );
					pdc.media.render_timout = setTimeout(function(){
						
						pdc.do_action('append', self.$el);
				...
				save: function( e ) {
					...
					// serialize form
					var data = pdc.serialize_form(this.$el);
...
(function($){
	...
	pdc.fields.oembed = pdc.field.extend({
		...
		*/
		...
		focus: function(){
			...
			// vars
			this.$el = this.$field.find('.pdc-oembed');
		...
		*/
		...
		search: function(){ 
			...
			// AJAX data
			var ajax_data = pdc.prepare_for_ajax({
...
(function($){
	...
	pdc.fields.radio = pdc.field.extend({
		...
		focus: function(){
			...
			// focus on $select
			this.$ul = this.$field.find('.pdc-radio-list');
			
			
			// get options
			this.o = pdc.get_data( this.$ul );
...
(function($){
	...
	pdc.fields.relationship = pdc.field.extend({
		...
		events: {
			...
			'click .choices .pdc-rel-item': 	'add_item',
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-relationship');
			this.$input = this.$el.find('.pdc-hidden input');
...
*/
		...
		fetch: function(){
			...
			// add to this.o
			this.o.action = 'pdc/fields/relationship/query';
			this.o.field_key = $field.data('key');
			this.o.post_id = pdc.get('post_id');
			
			
			// ready for ajax
			var ajax_data = pdc.prepare_for_ajax( this.o );
			...
			// add message
			this.$choices.find('ul:last').append('<p><i class="pdc-loading"></i> ' + pdc._e('relationship', 'loading') + '</p>');
		...
		render: function( json ){
			...
			if( !json || !json.results || !json.results.length ) {
				...
				if( this.o.paged == 1 ) {
					...
					this.$choices.children('.list').append('<p>' + pdc._e('relationship', 'empty') + '</p>');
			...
			this.$values.find('.pdc-rel-item').each(function(){
				
				$new.find('.pdc-rel-item[data-id="' +  $(this).data('id') + '"]').addClass('disabled');
			...
			if( this.o.s ) {
				...
				// loop
				$new.find('.pdc-rel-item').each(function(){
			...
			this.$choices.find('.pdc-rel-label').each(function(){
		...
		walker: function( data ){
			...
			if( $.isArray(data) ) {
				...
				if( data.children !== undefined ) {
					...
					s += '<li><span class="pdc-rel-label">' + data.text + '</span><ul class="pdc-bl">';
					...
					s += '<li><span class="pdc-rel-item" data-id="' + data.id + '">' + data.text + '</span></li>';
		...
		add_item: function( e ){
			...
			if( this.o.max > 0 ) {
				...
				if( this.$values.find('.pdc-rel-item').length >= this.o.max ) {
				
					alert( pdc._e('relationship', 'max').replace('{max}', this.o.max) );
			...
			var html = [
				'<li>',
					...
					'<span data-id="' + e.$el.data('id') + '" class="pdc-rel-item">' + e.$el.html(),
			...
			// validation
			pdc.validation.remove_error( this.$field );
		...
		remove_item : function( e ){
			...
			// show
			this.$choices.find('.pdc-rel-item[data-id="' + id + '"]').removeClass('disabled');
	...
	pdc.select2 = pdc.model.extend({
		...
		*/
		...
		ready: function(){
			...
			if( pdc.maybe_get(window, 'Select2') ) {
				
				this.version = 3;
				
				this.l10n_v3();
				
			} else if( pdc.maybe_get(window, 'jQuery.fn.select2.amd') ) {
		...
		*/
		...
		l10n_v3: function(){
			...
			$.fn.select2.locales = pdc.maybe_get(window, 'jQuery.fn.select2.locales', {});
		...
		*/
		...
		test1: function(){
			...
			// vars
			var $select1 = $('#pdc-field_58f402c1bbcfc'),
				$select2 = $('#pdc-field_58f409669fb72');
			...
			pdc.select2.set_data( $select1, item );
			pdc.select2.set_data( $select2, item );
		...
		test3: function(){
			...
			pdc.select2.set_data( $select1, items );
			pdc.select2.set_data( $select2, items );
		...
		*/
		...
		decode_data: function( data ) {
			...
			$.each(data, function(k, v){
				...
				// text
				data[ k ].text = pdc.decode( v.text );
				
				
				// children
				if( typeof v.children !== 'undefined' ) {
					
					data[ k ].children = pdc.select2.decode_data(v.children);
		...
		*/
		...
		get_ajax_data: function( args, params, $el, $field ){
			...
			// filter
			data = pdc.apply_filters( 'select2_ajax_data', data, args, $el, $field );
		...
		*/
		...
		get_ajax_results: function( data, params ){
			...
			// filter
			data = pdc.apply_filters( 'select2_ajax_results', data, params );
		...
		*/
		...
		init_v3: function( $select, args, $field ){
			...
			if( args.multiple ) {
				...
				value = pdc.maybe_get(value, 0, false);
			...
			if( args.ajax ) {
				...
				select2_args.ajax = {
					...
					data: function( term, page ) {
						...
						// return
						return pdc.select2.get_ajax_data(args, params, $input, $field);
					...
					results: function( data, page ){
						...
						setTimeout(function(){
							...
							pdc.select2.merge_results_v3();
							
						}, 1);
						
						
						// return
						return pdc.select2.get_ajax_results(data, params);
			...
			// append args
			select2_args.pdc = args;
			
			
			// filter for 3rd party customization
			select2_args = pdc.apply_filters( 'select2_args', select2_args, $select, args, $field );
			...
			// disbale select
			$select.prop('disabled', true).addClass('pdc-disabled pdc-hidden');
			...
			// action for 3rd party customization
			pdc.do_action('select2_init', $input, select2_args, args, $field);
			...
			if( !args.ajax ) {
				...
				select2_args.ajax = {
					...
					data: function( params ) {
						...
						// return
						return pdc.select2.get_ajax_data(args, params, $select, $field);
						
					},
					processResults: function( data, params ){
						
						// vars
						var results = pdc.select2.get_ajax_results(data, params);
...
*/
			...
			// action for 3rd party customization
			pdc.do_action('select2_init', $select, select2_args, args, $field);
		...
		*/
		...
		destroy: function( $select ){
			...
			// enable select
			$select.prop('disabled', false).removeClass('pdc-disabled pdc-hidden');
	...
	*/
	...
	pdc.add_select2 = function( $select, args ) {
		
		pdc.select2.init( $select, args );

	}
	
	pdc.remove_select2 = function( $select ) {
		
		pdc.select2.destroy( $select );
		
	}
	
	
	// select
	pdc.fields.select = pdc.field.extend({
		...
		focus: function(){
			...
			// get options
			this.o = pdc.get_data( this.$select );
			
			
			// customize o
			this.o = pdc.parse_args(this.o, {
				'ajax_action':	'pdc/fields/'+this.type+'/query',
		...
		render: function(){
			...
			pdc.select2.init( this.$select, this.o, this.$field );
		...
		remove: function(){
			...
			// remove select2
			pdc.select2.destroy( this.$select );
	...
	// user
	pdc.fields.user = pdc.fields.select.extend({
	...
	// post_object
	pdc.fields.post_object = pdc.fields.select.extend({
	...
	// page_link
	pdc.fields.page_link = pdc.fields.select.extend({
...
(function($){
	...
	pdc.fields.tab = pdc.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-tab');
		...
		hide: function( $field, context ){
			...
			// vars
			var key = $field.data('key'),
				$group = $field.prevAll('.pdc-tab-wrap'),
			...
			$field.nextUntil('.pdc-field-tab', '.pdc-field').each(function(){
				...
				// hide field
				pdc.conditional_logic.hide_field( $(this) );
		...
		show: function( $field, context ){
			...
			$field.siblings('.pdc-field.-hbcl-' + key).each(function(){
				
				// show field
				pdc.conditional_logic.show_field( $(this) );
	...
	*/
	...
	var tab_manager = pdc.model.extend({
		...
		add_group: function( $field, settings ){
			...
			// add sidebar to wrap
			if( $wrap.hasClass('pdc-fields') && settings.placement == 'left' ) {
			...
			if( $wrap.is('tbody') ) {
				...
				html = '<tr class="pdc-tab-wrap"><td colspan="2"><ul class="pdc-hl pdc-tab-group"></ul></td></tr>';
			
			} else {
			
				html = '<div class="pdc-tab-wrap -' + settings.placement + '"><ul class="pdc-hl pdc-tab-group"></ul></div>';
		...
		add_tab: function( $field, settings ){ //console.log('add_tab(%o, %o)', $field, settings);
			...
			// vars
			var $group = $field.siblings('.pdc-tab-wrap').last();
			...
			// vars
			var $li = $('<li><a class="pdc-tab-button" href="#" data-key="' + settings.key + '">' + settings.text + '</a></li>');
		...
		_click: function( e ){
			...
			$group.nextUntil('.pdc-tab-wrap', '.pdc-field').each(function(){
				...
				if( current === show ) {
					...
					if( $field.hasClass('hidden-by-tab') ) {
						...
						pdc.do_action('show_field', $(this), 'tab');
				...
				// hide
					...
					if( !$field.hasClass('hidden-by-tab') ) {
						...
						pdc.do_action('hide_field', $(this), 'tab');
			...
			// action for 3rd party customization
			pdc.do_action('refresh', $group.parent() );
	...
	*/
	...
	var tab_validation = pdc.model.extend({
		...
		add_field_error: function( $field ){
			...
			// vars
			var $tab = $field.prevAll('.pdc-field-tab:first'),
				$group = $field.prevAll('.pdc-tab-wrap:first');
...
(function($){
	...
	pdc.fields.time_picker = pdc.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.pdc-time-picker');
			this.$input = this.$el.find('input[type="text"]');
			this.$hidden = this.$el.find('input[type="hidden"]');
		...
		initialize: function(){
			...
			var args = {
				...
				controlType: 		'select',
				oneLine:			true,
				closeText:			pdc._e('date_time_picker', 'selectText')
			};
			...
			args.onClose = function( value, instance ){
				...
				if( !value && $close.is(':hover') ) {
					...
					value = pdc.maybe_get(instance, 'settings.timepicker.formattedTime');
			...
			// filter for 3rd party customization
			args = pdc.apply_filters('time_picker_args', args, this.$field);
			...
			// action for 3rd party customization
			pdc.do_action('time_picker_init', this.$input, args, this.$field);
...
(function($){
	...
	pdc.fields.true_false = pdc.field.extend({
		...
		events: {
			'change .pdc-switch-input': '_change',
			'focus .pdc-switch-input': 	'_focus',
			'blur .pdc-switch-input': 	'_blur',
			'keypress .pdc-switch-input':	'_keypress'
		},
		...
		*/
		...
		focus: function(){
			...
			// vars
			this.$input = this.$field.find('.pdc-switch-input');
			this.$switch = this.$field.find('.pdc-switch');
		...
		*/
		...
		render: function(){
			...
			// vars
			var $on = this.$switch.children('.pdc-switch-on'),
				$off = this.$switch.children('.pdc-switch-off')
...
(function($){
	...
	// taxonomy
	pdc.fields.taxonomy = pdc.field.extend({
		...
		focus: function(){
			...
			// $el
			this.$el = this.$field.find('.pdc-taxonomy-field');
		...
		render: function(){
			...
			// select2 options
			var args = pdc.get_data( $select );
			
			
			// customize args
			args = pdc.parse_args(args, {
				'pagination':	true,
				'ajax_action':	'pdc/fields/taxonomy/query',
				'key':			this.o.key
			});
		...
		add_term_confirm: function( html ){
			...
			// focus
			$('#pdc-popup input[name="term_name"]').focus();
			
			
			// events
			$('#pdc-popup form').on('submit', function( e ){
		...
		add_term_submit: function( $form ){
			...
			// vars
			var $submit = $form.find('.pdc-submit'),
			...
			$submit.find('.pdc-spinner').addClass('is-active');
			...
			$.ajax({
				...
				complete: function(){
					...
					// hide loading
					$submit.find('.pdc-spinner').removeClass('is-active');
		...
		append_new_term: function( term ){
			...
			$('.pdc-taxonomy-field[data-taxonomy="' + this.o.taxonomy + '"]').each(function(){
			...
			// append to select
			$('#pdc-popup #term_parent').each(function(){
			...
			switch( this.o.type ) {
				...
				case 'select':
					...
					//pdc.select2.set_data($select, item);
				...
				case 'multi_select':
					...
					// vars
					//var $select = this.$el.children('select');
					//pdc.select2.append_data($select, item);
...
(function($){
	...
	pdc.fields.url = pdc.field.extend({
...
(function($){
	...
	pdc.validation = pdc.model.extend({
		...
		events: {
			...
			'click .pdc-error-message a':	'click_message'
		},
		...
		// classes
		error_class: 'pdc-error',
		message_class: 'pdc-error-message',
		...
		*/
		...
		ready: function( $el ){
			...
			// reference
			$el.find('.pdc-field input').filter('[type="number"], [type="email"], [type="url"]').on('invalid', function( e ){
				...
				// run validation
				pdc.validation.fetch( $(this).closest('form') );
		...
		*/
		...
		submit_form: function( e ){
			...
			// bail early if this form does not contain pdc data
			if( !e.$el.find('#pdc-form-data').exists() ) {
		...
		*/
		...
		toggle: function( $form, state ){
			...
			if( !$parent.exists() ) {
				...
				$parent = $form.find('.pdc-form-submit');
			...
			$spinner = $parent.find('.spinner, .pdc-spinner');
		...
		*/
		...
		fetch: function( $form ){
			...
			pdc.do_action('validation_begin');
				
				
			// vars
			var data = pdc.serialize_form($form);
		...
		*/
		...
		fetch_complete: function( $form ){
			...
			var $message = $form.children('.pdc-error-message');
			
			if( $message.exists() ) {
				
				$message.addClass('-success');
				$message.children('p').html( pdc._e('validation_successful') );
				
				
				// remove message
				setTimeout(function(){
					
					pdc.remove_el( $message );
			...
			$form.find('.pdc-postbox.pdc-hidden').remove();
			
			
			// action for 3rd party customization
			pdc.do_action('submit', $form);
		...
		*/
		...
		fetch_success: function( $form, json ){
			...
			// filter for 3rd party customization
			json = pdc.apply_filters('validation_complete', json, $form);
			...
			if( !json || json.valid || !json.errors ) {
				...
				// action for 3rd party
				pdc.do_action('validation_success');
			...
			// action for 3rd party
			pdc.do_action('validation_failure');
			...
			var $scrollTo = null,
				count = 0,
				message = pdc._e('validation_failed');
			...
			if( json.errors && json.errors.length > 0 ) {
				...
				for( var i in json.errors ) {
					...
					// now get field
					var $field = pdc.get_field_wrap( $input );
				...
				if( count == 1 ) {
					...
					message += '. ' + pdc._e('validation_failed_1');
					
				} else if( count > 1 ) {
					
					message += '. ' + pdc._e('validation_failed_2').replace('%d', count);
			...
			if( !$message.exists() ) {
				...
				$message = $('<div class="pdc-error-message"><p></p><a href="#" class="pdc-icon -cancel small"></a></div>');
		...
		*/
		...
		add_error: function( $field, message ){
			...
			if( message !== undefined ) {
				...
				$field.children('.pdc-input').children('.' + this.message_class).remove();
				$field.children('.pdc-input').prepend('<div class="' + this.message_class + '"><p>' + message + '</p></div>');
			...
			pdc.do_action('add_field_error', $field);
		...
		*/
		...
		remove_error: function( $field ){
			...
			// var
			var $message = $field.children('.pdc-input').children('.' + this.message_class);
			...
			// hook for 3rd party customization
			pdc.do_action('remove_field_error', $field);
		...
		*/
		...
		add_warning: function( $field, message ){
			...
			setTimeout(function(){
				...
				pdc.validation.remove_error( $field )
		...
		*/
		...
		show_spinner: function( $spinner ){
			...
			// vars
			var wp_version = pdc.get('wp_version');
...
(function($){
	...
	pdc.fields.wysiwyg = pdc.field.extend({
		...
		events: {
			'mousedown .pdc-editor-wrap.delay': 'mousedown'
		},
		...
		mousedown: function(e) {
			...
			this.$el.find('.pdc-editor-toolbar').remove();
		...
		initialize: function(){
			...
			var old_id = this.o.id,
				new_id = pdc.get_uniqid('pdc-editor-'),
				html = this.$el.outerHTML();
			
			
			// replace
			html = pdc.str_replace( old_id, new_id, html );
		...
		initialize_tinymce: function(){
			...
			try {
				...
				// action for 3rd party customization
				pdc.do_action('wysiwyg_tinymce_init', ed, ed.id, mceInit, this.$field);
		...
		initialize_quicktags: function(){
			...
			try {
				...
				// action for 3rd party customization
				pdc.do_action('wysiwyg_quicktags_init', qtag, qtag.id, qtInit, this.$field);
		...
		get_mceInit : function(){
			...
			var toolbar = this.get_toolbar( this.o.toolbar ),
				mceInit = $.extend({}, tinyMCEPreInit.mceInit.pdc_content);
			...
			if( toolbar ) {
				...
				for( var i = 1; i < 5; i++ ) {
					...
					mceInit[ k + i ] = pdc.isset(toolbar, i) ? toolbar[i] : '';
...
*/
			...
			// hook for 3rd party customization
			mceInit = pdc.apply_filters('wysiwyg_tinymce_settings', mceInit, mceInit.id, this.$field);
		...
		get_qtInit : function(){
			...
			// vars
			var qtInit = $.extend({}, tinyMCEPreInit.qtInit.pdc_content);
			...
			// hook for 3rd party customization
			qtInit = pdc.apply_filters('wysiwyg_quicktags_settings', qtInit, qtInit.id, this.$field);
	...
	*/
	...
	var pdc_content = pdc.model.extend({
...
// @codekit-prepend "../js/pdc-ajax.js";
// @codekit-prepend "../js/pdc-checkbox.js";
// @codekit-prepend "../js/pdc-color-picker.js";
// @codekit-prepend "../js/pdc-conditional-logic.js";
// @codekit-prepend "../js/pdc-date-picker.js";
// @codekit-prepend "../js/pdc-date-time-picker.js";
// @codekit-prepend "../js/pdc-file.js";
// @codekit-prepend "../js/pdc-google-map.js";
// @codekit-prepend "../js/pdc-image.js";
// @codekit-prepend "../js/pdc-media.js";
// @codekit-prepend "../js/pdc-oembed.js";
// @codekit-prepend "../js/pdc-radio.js";
// @codekit-prepend "../js/pdc-relationship.js";
// @codekit-prepend "../js/pdc-select.js";
// @codekit-prepend "../js/pdc-tab.js";
// @codekit-prepend "../js/pdc-time-picker.js";
// @codekit-prepend "../js/pdc-true-false.js";
// @codekit-prepend "../js/pdc-taxonomy.js";
// @codekit-prepend "../js/pdc-url.js";
// @codekit-prepend "../js/pdc-validation.js";
// @codekit-prepend "../js/pdc-wysiwyg.js";
