...
class pdc_field_color_picker extends pdc_field {
	...
	*/
	...
	function render_field( $field ) {
		...
		// vars
		$text = pdc_get_sub_array( $field, array('id', 'class', 'name', 'value') );
		$hidden = pdc_get_sub_array( $field, array('name', 'value') );
		$e = '';
		
		
		// render
		?>
		<div class="pdc-color_picker">
			<?php pdc_hidden_input($hidden); ?>
			<input type="text" <?php echo pdc_esc_attr($text); ?> />
		</div>
		<?php
	}
...
// initialize
pdc_register_field_type( new pdc_field_color_picker() );
