...
class PDCTC_Core {
    ...
    public function __construct($plugin_path, $plugin_url, $plugin_basename, $plugin_version) {
        ...
        // Hooks
        //add_action( 'admin_init', array($this, 'pdc_theme_code_pro_check') );
    ...
    /**
     * Check if pdc Theme Code Pro is activated
     */
    public function pdc_theme_code_pro_check() {

        // If Theme Code Pro is activated, disable Theme Code (free) and display notice
        if (is_plugin_active('pdc-theme-code-pro/pdc_theme_code_pro.php')) {
    ...
    /**
     * pdc Theme Code (free) disabled notice
     */
    ...
    /**
     * Check if pdc plugin is free or pro version
     */
    public function set_db_table() {

        // If we can't detect pdc
        if (!class_exists('pdc')) {
            // bail early
            return;
        }

        // Check for the function pdc_get_setting - this came in version 5
        if (function_exists('pdc_get_setting')) {
            ...
            $version = apply_filters('pdc/get_info', 'version');
        }
    ...
    public function register_meta_boxes() {
        add_meta_box(
            'pdctc-meta-box', __('Theme Code', 'pdc'), array($this, 'display_callback'), array('pdc', 'pdc-field-group') // same meta box used for pdc and pdc PRO
        );
    }
    ...
    public function display_callback($post) {
        if (self::$db_table == 'postmeta') {
            ...
            if ($location_rules_count >= 2) {
                echo '<div class="pdctc-pro-notice pdctc-pro-notice--top"><a class="pdctc-pro-notice__link" href="https://hookturn.io/downloads/pdc-theme-code-pro/?utm_source=pdctclocation" target="_blank">Generate code for each of your Location rules with <strong>pdc Theme Code Pro</strong></a></div>';
            }

            // render the field group
            $parent_field_group = new pdcTC_Group($post->ID);
        }
        elseif (self::$db_table == 'posts') {
            ...
            // render the field group
            $parent_field_group = new pdcTC_Group($post->ID, 0, 0, '');
        }
    ...
    public function enqueue($hook) {
        ...
        // if post type is an pdc field group
        if ('pdc-field-group' == $post_type || 'pdc' == $post_type) {

            // Plugin styles
            wp_enqueue_style('pdctc_css', self::$plugin_url . 'assets/pdc-theme-code.css', '', self::$plugin_version);

            // Prism (code formatting)
            wp_enqueue_style('pdctc_prism_css', self::$plugin_url . 'assets/prism.css', '', self::$plugin_version);
            wp_enqueue_script('pdctc_prism_js', self::$plugin_url . 'assets/prism.js', '', self::$plugin_version);

            // Clipboard
            wp_enqueue_script('pdctc_clipboard_js', self::$plugin_url . 'assets/clipboard.min.js', '', self::$plugin_version);

            // Plugin js
            wp_enqueue_script('pdctc_js', self::$plugin_url . 'assets/pdc-theme-code.js', array('pdctc_clipboard_js'), self::$plugin_version, 'true');
        }
    }
