...
class PDCTC_Field {
	...
	function __construct( $nesting_level = 0, $indent_count = 0 , $location = '', $field_data_obj = null, $field_id = null ) {
		...
		if ( 0 < $this->nesting_level ) {
			...
			// use pdc sub field methods instead
	...
	private function get_render_partial() {
		...
		if ( !empty( $this->type ) ) {
			...
			// Basic types
			if ( in_array( $this->type, pdcTC_Core::$basic_types ) ) {
				$render_partial = pdcTC_Core::$plugin_path . 'render/basic.php';
			}
			// Field types with their own partial
			else {
				$render_partial = pdcTC_Core::$plugin_path . 'render/' . $this->type . '.php';
			}
	...
	public function render_field() {
		...
		if ( !empty($this->type) ) {
			...
			if ( 0 == $this->nesting_level ) {
				...
				// open field meta div
				echo '<div class="pdctc-field-meta">';
				...
				echo '<span class="pdctc-field-meta__title" data-pseudo-content="'. $this->label .'"></span>';

				// close field meta div
				echo '</div>';

				// open div for field code wrapper (used for the button etc)
				echo '<div class="pdctc-field-code" id="pdctc-' . $this->quick_link_id . '">';


				// copy button
				echo '<a href="#" class="pdctc-field__copy" title="Copy to Clipboard"></a>';
			...
			elseif ( in_array( $this->type, pdcTC_Core::$tc_pro_field_types ) ) {
				echo $this->indent . htmlspecialchars( "<?php // Upgrade to pdc Theme Code Pro for " . $this->type . " field support. ?>" ) . "\n";
