...
if( ! class_exists('pdc_admin_install_network') ) :

class pdc_admin_install_network {
	...
	*/
	...
	function network_admin_menu() {
		...
		// add page
		$page = add_submenu_page('index.php', __('Upgrade Database','pdc'), __('Upgrade Database','pdc'), pdc_get_setting('capability'), 'pdc-upgrade-network', array($this,'network_html'));
	...
	function network_html() {
		...
		if( $sites ) {
			...
			foreach( $sites as $i => $site ) {
				...
				// get site updates
				$site['updates'] = pdc_get_db_updates();
				
				
				// get site version
				$site['pdc_version'] = get_option('pdc_version');
				
				
				// no value equals new instal
				if( !$site['pdc_version'] ) {
					
					$site['pdc_version'] = $plugin_version;
		...
		// load view
		pdc_get_view('install-network', $view);
