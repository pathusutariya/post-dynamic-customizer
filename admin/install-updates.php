...
*  pdc_update_500
*
*  These functions will update the DB for pdc v5.0.0
*
*  @type	function
*  @date	10/09/2016
*  @since	5.4.0
...
function pdc_update_500() {
	
	// action for 3rd party
	do_action('pdc/update_500');
	
	
	// field groups
	pdc_update_500_field_groups();
...
function pdc_update_500_field_groups() {
	...
	foreach( $ofgs as $ofg ){
		...
		$nfg = pdc_update_500_field_group( $ofg );
	 		
	}
	
}

function pdc_update_500_field_group( $ofg ) {
	...
	// Note: pdc_update_field_group will call the pdc_get_valid_field_group function and apply 'compatibility' changes
	...
	// action for 3rd party
	do_action('pdc/update_500_field_group', $nfg, $ofg);
	...
	if( $rows ) {
		...
		foreach( $rows as $row ) {
			...
			if( pdc_has_done('update_500_field_group_' . $ofg->ID . '_' . $row['meta_key']) ) continue;
			...
			// migrate field
			$field = pdc_update_500_field( $field );
...
function pdc_update_500_field( $field ) {
	...
	$field['menu_order'] = pdc_extract_var( $field, 'order_no' );
	...
	if( $field['type'] == 'repeater' ) {
		...
		// get sub fields
		$sub_fields = pdc_extract_var( $orig, 'sub_fields' );
		...
		if( !empty($sub_fields) ) {
			...
			foreach( $keys as $key ) {
				...
				$sub_field = pdc_extract_var($sub_fields, $key);
				$sub_field['parent'] = $field['ID'];
				
				pdc_update_500_field( $sub_field );
		...
		// get layouts
		$layouts = pdc_extract_var( $orig, 'layouts' );
...
*  pdc_update_550
*
*  These functions will update the DB for pdc v5.5.0
*
*  @type	function
*  @date	10/09/2016
*  @since	5.4.0
...
function pdc_update_550() { //pdc_log('pdc_update_550');
	
	// action for 3rd party
	do_action('pdc/update_550');
	
	
	// termmeta
	pdc_update_550_termmeta();
...
function pdc_update_550_termmeta() { //pdc_log('pdc_update_550_termmeta');
	
	// bail early if no table
	if( !pdc_isset_termmeta() ) {
		
		update_option('pdc_update_550_termmeta', 1); // no longer used
		//echo __('Term meta upgrade not possible (termmeta table does not exist)', 'pdc');
		return;
	...
	foreach( $taxonomies as $taxonomy ) {
		...
		pdc_update_550_taxonomy( $taxonomy->name );
		
	}
	
	
	// delete trigger
	delete_option('pdc_update_550_termmeta');
	
	
	// action for 3rd party
	do_action('pdc/update_550_termmeta');
	
}


/*
*  pdc_update_550_taxonomy
*
...
function pdc_update_550_taxonomy( $taxonomy ) { //pdc_log('pdc_update_550_taxonomy', $taxonomy);
	...
	// action for 3rd party
	do_action('pdc/update_550_taxonomy', $taxonomy);
