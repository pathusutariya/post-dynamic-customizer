...
<div id="pdc-upgrade-wrap" class="wrap">
	...
	<p><?php echo sprintf( __("The following sites require a DB upgrade. Check the ones you want to update and then click %s.", 'pdc'), '"' . $button . '"'); ?></p>
	...
	<table class="wp-list-table widefat">
		...
		<thead>
			<tr>
				...
				<th class="manage-column" scope="col" style="width:33%;"><label for="sites-select-all"><?php _e("Site", 'pdc'); ?></label></th>
		...
		<tr>
			...
			<th class="manage-column" scope="col"><label for="sites-select-all-2"><?php _e("Site", 'pdc'); ?></label></th>
			<th><?php _e("Description", 'pdc'); ?></th>
		</tr>
		</tfoot>
		...
		<?php foreach( $sites as $i => $site ): ?>
			...
			<tr<?php if( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<td>
				<?php if( $site['updates'] ): ?>
					<span class="response"><?php printf(__('Site requires database upgrade from %s to %s', 'pdc'), $site['pdc_version'], $plugin_version); ?></span>
				<?php else: ?>
	...
	(function($) {
		...
		var upgrader = {
			...
			upgrade: function(){
				...
				// add loading
				$tr.find('.response').html('<i class="pdc-loading"></i></span> <?php printf(__('Upgrading data to version %s', 'pdc'), $plugin_version); ?>');
