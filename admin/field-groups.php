...
class pdc_admin_field_groups {
	...
	*/
	...
	function check_duplicate() {
		...
		// message
		if( $ids = pdc_maybe_get($_GET, 'pdcduplicatecomplete') ) {
		...
		// check single
		if( $id = pdc_maybe_get($_GET, 'pdcduplicate') ) {
			
			$ids[] = $id;
		
		// check multiple
		} elseif( pdc_maybe_get($_GET, 'action2') === 'pdcduplicate' ) {
			
			$ids = pdc_maybe_get($_GET, 'post');
	...
	*/
	...
	function check_sync() {
		...
		// message
		if( $ids = pdc_maybe_get($_GET, 'pdcsynccomplete') ) {
		...
		// check single
		if( $key = pdc_maybe_get($_GET, 'pdcsync') ) {
			
			$sync_keys[] = $key;
		
		// check multiple
		} elseif( pdc_maybe_get($_GET, 'action2') === 'pdcsync' ) {
			
			$sync_keys = pdc_maybe_get($_GET, 'post');
	...
	*/
	...
	function list_table_views( $views ) {
		...
		// active
		if( pdc_maybe_get($_GET, 'post_status') === 'sync' ) {
	...
	*/
	...
	function field_group_columns( $columns ) {
		...
		return array(
			...
			'pdc-fg-status' 		=> '<i class="pdc-icon -dot-3 small pdc-js-tooltip" title="' . __('Status', 'pdc') . '"></i>',
	...
	function render_column( $column, $field_group ) {
		...
		if( $column == 'pdc-fg-description' ) {
			...
			if( $field_group['description'] ) {
				...
				echo '<span class="pdc-description">' . $field_group['description'] . '</span>';
        ...
        // status
			...
			if( isset($this->sync[ $field_group['key'] ]) ) {
				...
				echo '<i class="pdc-icon -sync grey small pdc-js-tooltip" title="' . __('Sync available', 'pdc') .'"></i> ';
				
			}
			
			if( $field_group['active'] ) {
				
				//echo '<i class="pdc-icon -check small pdc-js-tooltip" title="' . __('Active', 'pdc') .'"></i> ';
				
			} else {
				
				echo '<i class="pdc-icon -minus yellow small pdc-js-tooltip" title="' . __('Inactive', 'pdc') . '"></i> ';
        ...
        // fields
			...
			echo pdc_get_field_count( $field_group );
...
?>
<script type="text/html" id="tmpl-pdc-column-2">
<div class="pdc-column-2">
	<div class="pdc-box">
		<div class="inner">
			<h2><?php echo pdc_get_setting('name'); ?></h2>
			<h3><?php _e("Resources",'pdc'); ?></h3>
			<ul>
				<li><a href="<?php echo $url_docs; ?>" target="_blank"><?php _e("Documentation",'pdc'); ?></a></li>
			</ul>
		</div>
		<div class="footer -blue">
			<p>This Plugin is Only For Private Use</p>
		</div>
	</div>
</div>
<div class="pdc-clear"></div>
</script>
...
(function($){
	...
	$('#the-list tr').each(function(){
		...
		if( id ) {
			...
			// create el
			var $span = $('<span class="pdc-duplicate-field-group"><a title="<?php _e('Duplicate this item', 'pdc'); ?>" href="<?php echo admin_url($this->url . '&pdcduplicate='); ?>' + post_id + '&_wpnonce=<?php echo wp_create_nonce('bulk-posts'); ?>"><?php _e('Duplicate', 'pdc'); ?></a> | </span>');
...
<?php foreach( $this->sync as $field_group ): $i++; ?>
	<tr <?php if($i%2 == 0): ?>class="alternate"<?php endif; ?>>
		<th class="check-column" scope="row">
			<label for="cb-select-<?php echo $field_group['key']; ?>" class="screen-reader-text"><?php printf( __( 'Select %s', 'pdc' ), $field_group['title'] ); ?></label>
			<input type="checkbox" value="<?php echo $field_group['key']; ?>" name="post[]" id="cb-select-<?php echo $field_group['key']; ?>">
		</th>
		<td class="post-title page-title column-title">
			<strong>
				<span class="row-title"><?php echo $field_group['title']; ?></span><span class="pdc-description"><?php echo $field_group['key']; ?>.json</span>
			</strong>
			<div class="row-actions">
				<span class="import"><a title="<?php echo esc_attr( __('Synchronise field group', 'pdc') ); ?>" href="<?php echo admin_url($this->url . '&post_status=sync&pdcsync=' . $field_group['key'] . '&_wpnonce=' . wp_create_nonce('bulk-posts')); ?>"><?php _e( 'Sync', 'pdc' ); ?></a></span>
			</div>
		</td>
