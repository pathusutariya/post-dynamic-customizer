...
if( ! class_exists('pdc_admin_settings_updates') ) :

class pdc_admin_settings_updates {
	...
	*/
	...
	function show_notice( $message = '', $class = '' ){
		...
		if( pdc_has_done('pdc_admin_settings_updates_notice') ) return false;
		
		
		// add notice
    	pdc_add_admin_notice( $message, $class );
	...
	*/
	...
	function show_error( $error = '' ){
    	...
    	if( is_wp_error($error) ) {
        	...
        	$error = __('<b>Error</b>. Could not connect to update server', 'pdc') . ' <span class="description">(' . esc_html( $error->get_error_message() ) . ')</span>';
	...
	function get_changelog_section( $changelog, $h4 = '' ) {
		...
		foreach( $bits as $bit ) {
	    	...
	    	if( version_compare($h4, $version, '==') ) {
	        	...
	        	return '<h4>' . esc_html($version) . '</h4>' . pdc_esc_html($text);
	...
	*/
	...
	function admin_menu() {
		...
		// add page
		//$page = add_submenu_page('edit.php?post_type=pdc-field-group', __('Updates','pdc'), __('Updates','pdc'), pdc_get_setting('capability'), 'pdc-settings-updates', array($this,'html') );
	...
	*/
	...
	function load() {
		...
		// activate
		if( pdc_verify_nonce('activate_pro_licence') ) {
			$this->activate_pro_licence();
		
		// deactivate	
		} elseif( pdc_verify_nonce('deactivate_pro_licence') ) {
		...
		$this->view = array(
			'license'			=> $license,
			'active'			=> $license ? 1 : 0,
			'current_version'	=> pdc_get_setting('version'),
			'remote_version'	=> '',
		...
		$info = pdc_updates()->get_plugin_info('pro', $force_check);
		...
		if( version_compare($info['version'], $version, '>') ) {
        	...
        	$update = pdc_updates()->get_plugin_update( $basename );
        	if( $license ) {
	        	if( !$update || $update['new_version'] !== $info['version'] ) {
		        	pdc_updates()->refresh_plugins_transient();
	        	}
        	}
        }
	}
	...
	*/
	...
	function activate_pro_licence() {
		...
		// connect
		$post = array(
			'pdc_license'	=> $_POST['pdc_pro_licence'],
		...
		// connect
		$response = pdc_updates()->request('v2/plugins/activate?p=pro', $post);
	...
	*/
	...
	function html() {
		...
		// load view
		pdc_get_view( dirname(__FILE__) . '/views/html-settings-updates.php', $this->view);
...
// initialize
new pdc_admin_settings_updates();
