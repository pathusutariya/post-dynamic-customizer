...
class pdc_admin_options_page {
	...
	*/
	...
	function admin_head() {
		...
		if( !empty($_GET['message']) && $_GET['message'] == '1' ) {
			...
			pdc_add_admin_notice( $this->page['updated_message'] );
	...
	*/
	...
	function postbox_pdc( $post, $args ) {
		...
		// render
		pdc_render_fields( $fields, $this->page['post_id'], 'div', $field_group['instruction_placement'] );
...
<?php
	...
	*/
	...
	function html() {
		...
		// load view
		pdc_get_view(dirname(__FILE__) . '/views/html-options-page.php', $this->page);
