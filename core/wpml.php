...
class pdc_wpml_compatibility {
	...
	*/
	...
	function __construct() {
		...
		if( isset($_POST['_pdclang']) ) $this->lang = $_POST['_pdclang'];
		
		
		// update settings
		pdc_update_setting('default_language', $sitepress->get_default_language());
		pdc_update_setting('current_language', $this->lang);
		...
		// actions
		add_action('pdc/update_500',					array($this, 'update_500'), 10);
		add_action('pdc/update_500_field_group',		array($this, 'update_500_field_group'), 10, 2);
		add_action('pdc/update_field_group',			array($this, 'update_field_group'), 2, 1);
		add_action('icl_make_duplicate',				array($this, 'icl_make_duplicate'), 10, 4);
		add_action('pdc/input/form_data',				array($this, 'pdc_input_form_data'), 10, 1);
	...
	*  is_translatable
	*
	*  This fucntion will return true if the pdc-field-group post type is translatable
	*
	*  @type	function
	*  @date	10/04/2015
	*  @since	5.2.3
	...
	*/
	...
	function is_translatable() {
		...
		// vars
		$post_types = pdc_maybe_get($sitepress_settings, 'custom_posts_sync_option', array());
		...
		// return true if pdc is translatable, and pdc-field-group does not yet exist
	...
	*  update_500
	*
	*  This function will update the WPML settings to allow 'pdc-field-group' to be translatable
	*
	*  @type	function
	*  @date	10/04/2015
	*  @since	5.2.3
	...
	*/
	...
	function update_500() {
		...
		// post type has changed from 'pdc' to 'pdc-field-group'
		if( !empty($post_types['pdc']) ) {
	...
	*/
	...
	function update_field_group( $field_group ) {
		...
		$this->lang = $sitepress->get_language_for_element($field_group['ID'], 'post_pdc-field-group');
	...
	*/
	...
	function admin_footer() {
		...
		(function($) {
			...
			// add filter
			pdc.add_filter('prepare_for_ajax', function( args ){
	...
	*/
	...
	function pdc_input_form_data( $data ) {
		...
		// add hidden input
		pdc_hidden_input(array('name' => '_pdclang', 'value' => $this->lang));
		
	}
	
}

new pdc_wpml_compatibility();
