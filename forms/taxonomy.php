...
*
*  @class 		pdc_form_taxonomy
*  @package		ACF
*  @subpackage	Forms
*/

if( ! class_exists('pdc_form_taxonomy') ) :
...
class pdc_form_taxonomy {
	...
	*/
	...
	function add_term( $taxonomy ) {
		...
		// vars
		$post_id = pdc_get_term_post_id( $taxonomy, 0 );
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				pdc_render_fields( $post_id, $fields, 'div', 'field' );
	...
	*/
	...
	function edit_term( $term, $taxonomy ) {
		...
		// vars
		$post_id = pdc_get_term_post_id( $term->taxonomy, $term->term_id );
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				<table class="form-table">
					<tbody>
						<?php pdc_render_fields( $post_id, $fields, 'tr', 'field' ); ?>
					</tbody>
				</table>
				<?php 
...
(function($) {
	...
	// update pdc validation class
	pdc.validation.error_class = 'form-invalid';
		
		
<?php if( $this->form == '#addtag' ): ?>

	// store origional HTML
	var $orig = $('#addtag').children('.pdc-field').clone();
	...
	$('#submit').on('click', function( e ){
		...
		if( !$('#addtag').find('#pdc-form-data').exists() ) {
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		// unlock form
		pdc.validation.toggle( $('#addtag'), 'unlock' );
		...
		pdc.do_action('remove', $('#addtag'));
		
		
		// remove old fields
		$('#addtag').find('.pdc-field').remove();
		
		
		// add orig fields
		$('#pdc-form-data').after( $orig.clone() );
		
		
		// action for 3rd party customization
		pdc.do_action('append', $('#addtag'));
...
<?php
	...
	*/
	...
	function save_term( $term_id, $tt_id, $taxonomy ) {
		...
		// vars
		$post_id = pdc_get_term_post_id( $taxonomy, $term_id );
		
		
		// verify and remove nonce
		if( !pdc_verify_nonce('taxonomy') ) return $term_id;
	...
	*/
	...
	function delete_term( $term, $tt_id, $taxonomy, $deleted_term ) {
		...
		if( pdc_isset_termmeta() ) return $term;
