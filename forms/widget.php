...
class pdc_form_widget {
	...
	function edit_widget( $widget, $return, $instance ) {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				pdc_render_fields( $post_id, $fields, 'div', $field_group['instruction_placement'] );
			...
			(function($) {
				...
				pdc.do_action('append', $('[id^="widget"][id$="<?php echo $widget->id; ?>"]') );
	...
	*/
	...
	function widget_update_callback( $instance, $new_instance, $old_instance, $widget ) {
		...
		// bail early if empty
		if( empty($_POST['pdc']) ) return $instance;
		
		
		// bail early if no nonce
		if( !pdc_verify_nonce('widget') ) return $instance;
		...
		if( !pdc_validate_save_post() ) return $instance;
		
		
		// save
		pdc_save_post( "widget_{$widget->id}" );	
	...
	*/
	...
	function customizer_widget_update_callback( $instance, $new_instance, $old_instance, $widget ) {
		...
		if( !pdc_validate_save_post() ) {
			...
			// message
			$message = __('Validation failed', 'pdc') . '. ';
			$message .= sprintf(_n( '1 field requires attention.', '%d fields require attention.', $total, 'pdc' ), $total);
			...
			// append pdc $_POST data to instance
			// this allows preview JS data to contain pdc values
			$instance['pdc']['values'] = $_POST['pdc'];
			
			
			// backup name => key reference
			// this will allow the customizer preview to correctly load the field when attempting to run pdc_load_value and pdc_format_value functions on newly added widgets 
			foreach( $_POST['pdc'] as $k => $v ) {
				...
				// update
				$instance['pdc']['reference'][ $field['name'] ] = $field['key'];
	...
	*/
	...
	function get_customizer_widgets( $customizer ) {
		...
		foreach( array_keys($settings) as $i ) {
			...
			// set data	
			$setting->pdc = pdc_maybe_get($value, 'pdc');
	...
	*/
	...
	function customize_preview_init( $customizer ) {
		...
		foreach( $widgets as $widget ) {
			...
			// bail early if no pdc
			if( empty($widget->pdc) ) continue;
			
			
			// append pdc_value to preview_values
			$this->preview_values[ $widget->pdc['ID'] ] = $widget->pdc['values'];
			$this->preview_reference[ $widget->pdc['ID'] ] = $widget->pdc['reference'];
		...
		// add filter
		add_filter('pdc/load_value', 			array($this, 'load_value'), 10, 3);
		add_filter('pdc/get_field_reference', 	array($this, 'get_field_reference'), 10, 3);
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {
		...
		// pdc/load_value filter is run first (before type, name, key)
		...
		if( isset($this->preview_values[ $post_id ][ $field['key'] ]) ) {
			...
			add_filter('pdc/load_value/key='.$field['key'], array($this, 'load_value_2'), 99, 3);
	...
	function load_value_2( $value, $post_id, $field ) {
		...
		remove_filter('pdc/load_value/key='.$field['key'], array($this, 'load_value_2'), 99, 3);
	...
	*/
	...
	function customize_save( $customizer ) {
		...
		foreach( $widgets as $widget ) {
			...
			// bail early if no pdc_value
			if( !$widget->pdc ) continue;
			
			// fake post data
			$_POST['pdc'] = $widget->pdc['values'];
			
			
			// save
			pdc_save_post( $widget->pdc['ID'] );
			...
			// remove [pdc] data from saved widget array
	...
	*  pre_update_option
	*
	*  this function will remove the [pdc] data from widget insance
	*
	*  @type	function
	*  @date	22/03/2016
	*  @since	5.3.2
	...
	*/
	...
	function pre_update_option( $value, $option, $old_value ) {
		...
		foreach( $value as $i => $widget ) {
			...
			// bail ealry if no pdc
			if( !isset($widget['pdc']) ) continue;
...
<script type="text/javascript">
(function($) {
	pdc.add_filter('get_fields', function( $fields ){
	 	...
	 	// customizer
	 	$fields = $fields.not('.widget-tpl .pdc-field');
	...
	$('#widgets-right').on('click', '.widget-control-save', function( e ){
		...
		if( pdc.validation.ignore ) {
		
			pdc.validation.ignore = 0;
			return true;
		...
		if( !$form.find('#pdc-form-data').exists() ) {
		...
		pdc.validation.$trigger = $(this);
		
		
		// run validation
		pdc.validation.fetch( $form );
	...
	$(document).on('widget-saved widget-updated', function( e, $widget ){
		...
		// unlock form
		pdc.validation.toggle( $widget, 'unlock' );
		
		
		// submit
		pdc.do_action('submit', $widget );
		
	});
	
	<?php if( pdc_is_screen('customize') ): ?>
	...
	WidgetControl._getInputsSignature = function( inputs ) {
		...
		for( var i in signature ) {
			...
			// bail ealry if pdc is found
			if( bit.indexOf('pdc') !== -1 ) continue;
