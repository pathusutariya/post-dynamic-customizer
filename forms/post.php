...
class pdc_form_post {
	...
	*/
	...
	function __construct() {
		...
		// ajax
		add_action('wp_ajax_pdc/post/get_field_groups',	array($this, 'get_field_groups'));
	...
	*/
	...
	function render_meta_box( $post, $args ) {
		...
		if( $visibility ) {
			...
			// render
			pdc_render_fields( $this->post_id, $fields, 'div', $field_group['instruction_placement'] );
...
<?php
	...
	*/
	...
	function get_field_groups() {
		...
		// vars
		$json = array();
		$exists = pdc_extract_var( $options, 'exists' );
		...
		// get field groups
		$field_groups = pdc_get_field_groups( $options );
		...
		foreach( $field_groups as $i => $field_group ) {
			...
			if( $i == 0 ) {
				...
				$item['style'] = pdc_get_field_group_style( $field_group );
			...
			if( !in_array($field_group['key'], $exists) ) {
				...
				// render
				pdc_render_fields( $options['post_id'], $fields, 'div', $field_group['instruction_placement'] );
	...
	*/
	...
	function wp_insert_post_empty_content( $maybe_empty, $postarr ) {
		...
		if( $maybe_empty && !empty($_POST['_pdcchanged']) ) {
	...
	*/
	...
	function allow_save_post( $post ) {
		...
		// vars
		$allow = true;
		$reject = array( 'auto-draft', 'revision', 'pdc-field', 'pdc-field-group' );
		$wp_preview = pdc_maybe_get($_POST, 'wp-preview');
	...
	*/
	...
	function save_post( $post_id, $post ) {
		...
		if( !pdc_verify_nonce('post', $post_id) ) return $post_id;
	...
	*/
	...
	function is_protected_meta( $protected, $meta_key, $meta_type ) {
		...
		// if pdc_get_field_reference returns a valid key, this is an pdc value, so protect it!
		if( !$protected ) {
			
			$reference = pdc_get_field_reference( $meta_key, $this->post_id );
