...
function pdc_get_setting( $name, $value = null ) {
	...
	return pdc()->get_setting( $name, $value );
...
function pdc_append_setting( $name, $value ) {
	...
	// vars
	$setting = pdc_get_setting( $name, array() );
	...
	// update
	pdc_update_setting( $name, $setting );
...
function pdc_has_done( $name ) {
	...
	if( pdc_get_setting($setting) ) return true;
	
	
	// update setting
	pdc_update_setting($setting, true);
...
function pdc_get_path( $path ) {
	...
	return pdc_get_setting('path') . $path;
...
function pdc_get_dir( $path ) {
	...
	return pdc_get_setting('dir') . $path;
...
function pdc_get_external_dir( $file, $path = '' ) {
    ...
    // vars
    $external_url = '';
    $external_path = pdc_get_external_path( $file, $path );
...
function pdc_get_view( $view_name = '', $args = array() ) {

	// vars
	$path = pdc_get_path("admin/views/{$view_name}.php");
...
function pdc_get_hidden_input( $atts ) {
	
	$atts['type'] = 'hidden';
	
	return '<input ' . pdc_esc_attr( $atts ) . ' />';
	
}

function pdc_hidden_input( $atts ) {
...
*  pdc_verify_nonce
*
*  This function will look at the $_POST['_pdcnonce'] value and return true or false
*
*  @type	function
*  @date	15/10/13
*  @since	5.0.0
...
function pdc_verify_nonce( $value, $post_id = 0 ) {
	
	// vars
	$nonce = pdc_maybe_get( $_POST, '_pdcnonce' );
	...
	if( $post_id ) {
		...
		// vars
		$form_post_id = (int) pdc_maybe_get( $_POST, 'post_ID' );
		...
		// 4. no match (this post is a custom created one during the save proccess via either WP or 3rd party)
			...
			// return false early and prevent $_POST['_pdcnonce'] from being reset
...
function pdc_verify_ajax() {
	...
	// bail early if not pdc action
	if( empty($_POST['action']) || substr($_POST['action'], 0, 3) !== 'pdc' ) {
	...
	if( empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'pdc_nonce') ) {
...
function pdc_get_pretty_taxonomies( $taxonomies = array() ) {
	...
	foreach( array_keys($taxonomies) as $i ) {
		...
		// vars
		$taxonomy = pdc_extract_var( $taxonomies, $i);
...
function pdc_get_term_title( $term ) {
...
function pdc_get_grouped_posts( $args ) {
	...
	// remove this filter (only once)
	remove_filter('posts_orderby', '_pdc_orderby_post_type');
	...
	foreach( $post_types as $post_type ) {
		...
		foreach( array_keys($posts) as $key ) {
			...
			if( $posts[ $key ]->post_type == $post_type ) {
				...
				$this_posts[] = pdc_extract_var( $posts, $key );
		...
		if( is_post_type_hierarchical($post_type) && empty($args['s'])) {
			...
			$parent = pdc_maybe_get( $args, 'post_parent', 0 );
		...
		foreach( array_keys($this_posts) as $key ) {
			...
			// extract post
			$post = pdc_extract_var( $this_posts, $key );
...
function pdc_str_replace( $string, $search_replace ) {
...
function pdc_maybe_get( $array, $key = 0, $default = null ) {
...
function pdc_get_attachment( $post ) {
	...
	$a = array(
		...
		'type'			=> pdc_maybe_get( explode('/', $post->post_mime_type), 0, '' ),
	...
	if( $a['type'] === 'image' ) {
		...
		if( $a['type'] == 'video' ) {
			...
			$a['width'] = pdc_maybe_get($meta, 'width', 0);
			$a['height'] = pdc_maybe_get($meta, 'height', 0);
...
function pdc_validate_attachment( $attachment, $field, $context = 'prepare' ) {
	...
	// filter for 3rd party customization
	$errors = apply_filters("pdc/validate_attachment", $errors, $file, $attachment, $field);
	$errors = apply_filters("pdc/validate_attachment/type={$field['type']}", $errors, $file, $attachment, $field );
	$errors = apply_filters("pdc/validate_attachment/name={$field['name']}", $errors, $file, $attachment, $field );
	$errors = apply_filters("pdc/validate_attachment/key={$field['key']}", $errors, $file, $attachment, $field );
...
function pdc_enable_filter( $filter = '' ) {
	...
	// get filters
	$filters = pdc_get_setting('_filters', array());
	...
	// update
	pdc_update_setting('_filters', $filters);
...
add_filter("pdc/settings/slug", '_pdc_settings_slug');

function _pdc_settings_slug( $v ) {
