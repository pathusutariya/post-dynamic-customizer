...
function pdc_get_field_reference( $field_name, $post_id ) {
	...
	// vars
	$field_key = pdc_get_metadata( $post_id, $field_name, true );
	
	
	// filter
	$field_key = apply_filters('pdc/get_field_reference', $field_key, $field_name, $post_id);
...
function get_field( $selector, $post_id = false, $format_value = true ) {
	...
	// get field
	$field = pdc_maybe_get_field( $selector, $post_id );
	...
	// get value for field
	$value = pdc_get_value( $post_id, $field );
	...
	if( $format_value ) {
		...
		// get value for field
		$value = pdc_format_value( $value, $post_id, $field );
...
function get_field_object( $selector, $post_id = false, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$field['value'] = pdc_format_value( $field['value'], $post_id, $field );
...
function get_field_objects( $post_id = false, $format_value = true, $load_value = true ) {
	...
	foreach( $meta as $k => $v ) {
		...
		$field = pdc_maybe_get_field( $field_key );
...
function have_rows( $selector, $post_id = false ) {
	...
	$active_loop = pdc_get_loop('active');
	$previous_loop = pdc_get_loop('previous');
	...
	// loop has changed
		...
		$sub_field = pdc_get_sub_field($selector, $active_loop['field']);
	...
	if( $new_parent_loop ) {
		...
		$value = pdc_extract_var( $field, 'value' );
...
function get_row( $format = false ) {
	...
	// get value
	$value = pdc_maybe_get( $loop['value'], $loop['i'] );
	...
	if( $format ) {
		...
		$value = pdc_format_value( $loop['value'], $loop['post_id'], $loop['field'] );
		
		
		// get value
		$value = pdc_maybe_get( $value, $loop['i'] );
...
function get_row_sub_field( $selector ) {
	...
	// attempt to find sub field
	$sub_field = pdc_get_sub_field($selector, $row['field']);
...
function get_sub_field_object( $selector, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$sub_field['value'] = pdc_format_value( $sub_field['value'], $row['post_id'], $sub_field );
...
function get_row_layout() {
	...
	// return
	if( isset($row['pdc_fc_layout']) ) {
		
		return $row['pdc_fc_layout'];
...
*  pdc_shortcode()
*
*  This function is used to add basic shortcode support for the pdc plugin
*  eg. [pdc field="heading" post_id="123" format_value="1"]
*
*  @type	function
*  @since	1.1.1
*  @date	29/01/13
...
add_shortcode('pdc', 'pdc_shortcode');
...
function update_field( $selector, $value, $post_id = false ) {
	...
	// get field
	$field = pdc_maybe_get_field( $selector, $post_id, false );
	...
	// save
	return pdc_update_value( $value, $post_id, $field );
...
function update_sub_field( $selector, $value, $post_id = false ) {
	...
	if( is_array($selector) ) {
		...
		$post_id = pdc_get_valid_post_id( $post_id );
		$sub_field = pdc_maybe_get_sub_field( $selector, $post_id, false );
		
	} else {
		
		$post_id = pdc_get_loop('active', 'post_id');
		$sub_field = get_row_sub_field( $selector );
	...
	// update
	return pdc_update_value( $value, $post_id, $sub_field );
...
function add_sub_row( $selector, $row = false, $post_id = false ) {
	...
	// get raw value
	$value = pdc_get_value( $post_id, $sub_field );
...
function pdc_filter_post_id( $post_id ) {
	
	return pdc_get_valid_post_id( $post_id );
