...
class pdc_revisions {
	...
	*/
	...
	function __construct() {
		...
		add_filter('pdc/validate_post_id', array($this, 'pdc_validate_post_id'), 10, 2 );
	...
	*/
	...
	function wp_preview_post_fields( $fields ) {
		...
		if( pdc_maybe_get_POST('wp-preview') !== 'dopreview' ) return $fields;
		
		
		// add to fields if PDC has changed
		if( pdc_maybe_get_POST('_pdc_changed') ) {
			
			$fields['_pdc_changed'] = 'different than 1';
	...
	*/
	...
	function wp_save_post_revision_check_for_changes( $return, $last_revision, $post ) {
		...
		// if pdc has changed, return false and prevent WP from performing 'compare' logic
		if( pdc_maybe_get_POST('_pdc_changed') ) return false;
	...
	*
	*  This filter will add the PDC fields to the returned array
	...
	*/
	...
	function wp_post_revision_fields( $fields, $post = null ) {
		...
		// validate page
		if( pdc_is_screen('revision') || pdc_is_ajax('get-revision-diffs') ) {
			
			// bail early if is restoring
			if( pdc_maybe_get_GET('action') === 'restore' ) return $fields;
		...
		foreach( $meta as $name => $value ) {
			...
			$ancestors = pdc_get_field_ancestors( $field );
			...
			if( !empty($ancestors) ) {
				...
				$oldest = pdc_get_field( $ancestors[$count-1] );
		...
		if( !empty($append) ) {
			...
			// add prefix
			$append = pdc_add_array_key_prefix($append, $prefix);
			$order = pdc_add_array_key_prefix($order, $prefix);
			...
			// remove prefix
			$append = pdc_remove_array_key_prefix($append, $prefix);
	...
	*/
	...
	function wp_post_revision_field( $value, $field_name, $post = null, $direction = false) {
		...
		// load field
		$field = pdc_maybe_get_field( $field_name, $post_id );
	...
	*/
	...
	function wp_restore_post_revision( $post_id, $revision_id ) {
		...
		pdc_copy_postmeta( $revision_id, $post_id );
		
		
		// Make sure the latest revision is also updated to match the new $post data
		// get latest revision
		$revision = pdc_get_post_latest_revision( $post_id );
		...
		if( $revision ) {
			...
			pdc_copy_postmeta( $revision_id, $revision->ID );
...
// initialize
pdc()->revisions = new pdc_revisions();

endif; // class_exists check


/*
*  pdc_save_post_revision
*
...
function pdc_save_post_revision( $post_id = 0 ) {
	...
	if( $revision ) {
		...
		pdc_copy_postmeta( $post_id, $revision->ID );
...
function pdc_get_post_latest_revision( $post_id ) {
