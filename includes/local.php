...
class pdc_local {
	...
	*/
	...
	function __construct() {
		...
		// actions
		add_action('pdc/include_fields', 	array($this, 'pdc_include_fields'), 5, 0);
		
		
		// filters
		add_filter('pdc/get_field_groups',	array($this, 'pdc_get_field_groups'), 20, 1);
	...
	*/
	...
	function is_enabled() {
		...
		if( !pdc_get_setting('local') ) return false;
		
		
		// return
		return pdc_is_filter_enabled('local');
	...
	*  is_ready
	*
	*  This function will return true when PDC has included all field types and is ready to import
	...
	*/
	...
	function is_ready() {
		...
		return did_action('pdc/include_fields');
	...
	*/
	...
	function get_field_groups() {
		...
		foreach( array_keys($this->groups) as $field_group_key ) {
			...
			$field_groups[] = pdc_get_field_group( $field_group_key );
	...
	*/
	...
	function get_fields( $parent_key = '' ) {
		...
		foreach( $this->parents[ $parent_key ] as $field_key ) {
			...
			$fields[] = pdc_get_field( $field_key );
	...
	*
	*  This function will override and add field groups to the `pdc_get_field_groups()` results
	*
	*  @type	filter (pdc/get_field_groups)
	*  @date	5/12/2013
	*  @since	5.0.0
	*
	...
	*/
	...
	function pdc_get_field_groups( $field_groups ) {
...
function pdc_disable_local() {
	
	pdc_disable_filter('local');
	
}

function pdc_enable_local() {
	
	pdc_enable_filter('local');
	
}

function pdc_reset_local() {
	
	return pdc_local()->reset();
	
}


// field group
function pdc_add_local_field_group( $field_group ) {
	
	return pdc_local()->maybe_add_field_group( $field_group );
	
}

function pdc_remove_local_field_group( $key = '' ) {
	
	return false;
	
}

function pdc_is_local_field_group( $key = '' ) {
	
	return pdc_local()->is_field_group( $key );
	
}

function pdc_get_local_field_group( $key = '' ) {
	
	return pdc_local()->get_field_group( $key );
	
}


// field groups
function pdc_count_local_field_groups() {
	
	return pdc_local()->count_field_groups();
	
}

function pdc_have_local_field_groups() {
	
	return pdc_local()->have_field_groups();
	
}

function pdc_get_local_field_groups() {
	
	return pdc_local()->get_field_groups();
	
}


// field
function pdc_add_local_field( $field ) {
	
	return pdc_local()->maybe_add_field( $field );
	
}

function pdc_remove_local_field( $key = '' ) {
	
	return pdc_local()->remove_field( $key );
	
}

function pdc_is_local_field( $key = '' ) {
	
	return pdc_local()->is_field( $key );
	
}

function pdc_is_local_field_key( $key = '' ) {
	
	return pdc_local()->is_field_key( $key );
	
}

function pdc_is_local_field_name( $name = '' ) {
	
	return pdc_local()->is_field_name( $name );
	
}

function pdc_get_local_field( $key = '' ) {
	
	return pdc_local()->get_field( $key );
	
}


// fields
function pdc_count_local_fields( $key = '' ) {
	
	return pdc_local()->count_fields( $key );
	
}

function pdc_have_local_fields( $key = '' ) {
	
	return pdc_local()->have_fields( $key );
	
}

function pdc_get_local_fields( $key = '' ) {
	
	return pdc_local()->get_fields( $key );
	
}

function pdc_remove_local_fields( $key = '' ) {
	
	return pdc_local()->remove_fields( $key );
