	...
	*/
	...
	function add_term( $taxonomy ) {
		...
		if( !empty($field_groups) ) {
			...
			// wrap
			echo '<div id="pdc-term-fields" class="pdc-fields -clear">';
			
			// loop
			foreach( $field_groups as $field_group ) {
				$fields = pdc_get_fields( $field_group );
				pdc_render_fields( $fields, $post_id, 'div', 'field' );
			}
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				echo '<table class="form-table">';
					$fields = pdc_get_fields( $field_group );
					pdc_render_fields( $fields, $post_id, 'tr', 'field' );
				echo '</table>';
...
if( $this->view == 'add' ): ?>
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		pdc.doAction('remove', $fields);
		
		// reset HTML
		$fields.html( html );
		
		// action for 3rd party customization
		pdc.doAction('append', $fields);
