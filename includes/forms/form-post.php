...
class PDC_Form_Post {
	...
	*/
	...
	function validate_page() {
		...
		// validate post type
		if( in_array($typenow, array('pdc-field-group', 'attachment')) ) {
	...
	function admin_head() {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $i => $field_group ) {
				...
				// visibility
				$args['visibility'] = pdc_get_field_group_visibility( $field_group, array(
				...
				if( !$style_found && $args['visibility'] ) {
					...
					$this->style = pdc_get_field_group_style( $field_group );
		...
		// Allow 'pdc_after_title' metabox position
		...
		// remove PDC values from meta postbox ()
	...
	*/
	...
	function edit_form_after_title() {
		...
		// render
		do_meta_boxes( get_current_screen(), 'pdc_after_title', $post);
		
		
		// clean up
		unset( $wp_meta_boxes['post']['pdc_after_title'] );
	...
	*/
	...
	function render_meta_box( $post, $args ) {
		...
		$o = array(
			...
			'editLink'		=> '',
			'editTitle'		=> __('Edit field group', 'pdc'),
		...
		if( $visibility ) {
			...
			// render
			pdc_render_fields( $fields, $this->post_id, 'div', $field_group['instruction_placement'] );
		
		// render replace-me div
		} else {
			
			echo '<div class="pdc-replace-with-fields"><div class="pdc-loading"></div></div>';
...
if( typeof pdc !== 'undefined' ) {
	...
	pdc.newPostbox(<?php echo json_encode($o); ?>);	
...
<?php
	...
	*/
	...
	function admin_footer(){
		...
		echo '<style type="text/css" id="pdc-style">' . $this->style . '</style>';
	...
	*
	*  This function will allow WP to insert a new post without title / content if PDC data exists
	*
	*  @type	function
	*  @date	16/07/2014
	*  @since	5.0.1
	...
	*/
	...
	function wp_insert_post_empty_content( $maybe_empty, $postarr ) {
		...
		if( $maybe_empty && pdc_maybe_get_POST('_pdc_changed') ) {
	...
	*/
	...
	function allow_save_post( $post ) {
		...
		$post_types = array( 'auto-draft', 'revision', 'pdc-field', 'pdc-field-group' );
		if( in_array($post->post_type, $post_types) ) $allow = false;
		
		
		// disallow if not the form post
		$form_post_id = (int) pdc_maybe_get_POST('post_ID');
		...
		if( $post->post_type == 'revision' ) {
			...
			if( pdc_maybe_get_POST('wp-preview') == 'dopreview' ) $allow = true;
	...
	*/
	...
	function save_post( $post_id, $post ) {
		...
		// verify nonce
		if( !pdc_verify_nonce('post') ) return $post_id;
		...
		if( post_type_supports($post->post_type, 'revisions') ) {
			...
			pdc_save_post_revision( $post_id );
	...
	*  is_protected_meta
	*
	*  This function will remove any PDC meta from showing in the meta postbox
	*
	*  @type	function
	*  @date	12/04/2014
	*  @since	5.0.0
	...
	*/
	...
	function is_protected_meta( $protected, $meta_key, $meta_type ) {
		...
		// if pdc_get_reference returns a valid key, this is an pdc value, so protect it!
		if( !$protected ) {
			
			$reference = pdc_get_reference( $meta_key, $this->post_id );
			
			if( pdc_is_field_key($reference) ) {
...
pdc_new_instance('PDC_Form_Post');
