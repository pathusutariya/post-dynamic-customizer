...
if( ! class_exists('pdc_form_customizer') ) :

class pdc_form_customizer {
	...
	*/
	...
	function save_widget( $instance, $new_instance, $old_instance, $widget ) {
		...
		// append values
		$data['values'] = $new_instance['pdc'];
		
		
		// append fields (name => key relationship) - used later in 'pdc/get_field_reference' for customizer previews
	...
	*  settings
	*
	*  This function will return an array of cutomizer settings that include PDC data
	...
	*/
	...
	function settings( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			if( !is_array($value) || !isset($value['pdc']) ) continue;
			
			
			// set data	
			$setting->pdc = $value['pdc'];
	...
	*/
	...
	function customize_preview_init( $customizer ) {
		...
		// add filters
		add_filter('pdc/pre_load_value', array($this, 'pre_load_value'), 10, 3);
		add_filter('pdc/pre_load_reference', array($this, 'pre_load_reference'), 10, 3);
	...
	*/
	...
	function customize_save( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// save pdc data
			pdc_save_post( $data['post_id'], $data['values'] );
...
(function($) {
	...
	// because PDC generates a unique ID on the WYSIWYG field, this string will not match causing the preview function to bail.
	...
	// either way, this function is not needed and will break PDC fields 
