...
/*
*  PDC Comment Form Class
*
*  All the logic for adding fields to comments
*
*  @class 		pdc_form_comment
*  @package		PDC
*  @subpackage	Forms
*/

if( ! class_exists('pdc_form_comment') ) :
...
class pdc_form_comment {
	...
	*  edit_comment
	*
	*  This function is run on the admin comment.php page and will render the PDC fields within custom metaboxes to look native
	*
	*  @type	function
	*  @date	19/10/13
	*  @since	5.0.0
	...
	function edit_comment( $comment ) {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				?>
				<div id="pdc-<?php echo $field_group['ID']; ?>" class="stuffbox">
					<h3 class="hndle"><?php echo $field_group['title']; ?></h3>
					<div class="inside">
						<?php pdc_render_fields( $fields, $post_id, 'div', $field_group['instruction_placement'] ); ?>
	...
	*/
	...
	function comment_form_field_comment( $html ) {
		...
		ob_start();
			...
			echo '<div class="pdc-comment-fields pdc-fields -clear">';
			
			foreach( $field_groups as $field_group ) {
				
				$fields = pdc_get_fields( $field_group );
				
				pdc_render_fields( $fields, $post_id, 'p', $field_group['instruction_placement'] );
	...
	*/
	...
	function save_comment( $comment_id ) {
	    ...
	    if( pdc_validate_save_post(true) ) {
			pdc_save_post( "comment_{$comment_id}" );
		}
