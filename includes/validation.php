...
class pdc_validation {
	...
	*/
	...
	function pdc_validate_save_post() {
		...
		// validate
		pdc_validate_values( $_POST['pdc'], 'pdc' );
...
*  Public functions
*
*  alias of pdc()->validation->function()
*
*  @type	function
*  @date	6/10/13
*  @since	5.0.0
...
function pdc_get_validation_error() {
	
	return pdc()->validation->get_error( $input );
...
function pdc_validate_save_post( $show_errors = false ) {
	...
	if( $show_errors ) {
		...
		// die
		wp_die( $message, __('Validation failed', 'pdc') );
...
function pdc_validate_values( $values, $input_prefix = '' ) {
...
function pdc_validate_value( $value, $field, $input ) {
	...
	*/
	...
	$valid = apply_filters( "pdc/validate_value/name={$field['_name']}", 	$valid, $value, $field, $input );
