...
if( ! class_exists('PDC_Updates') ) :

class PDC_Updates {
	
	/** @var string The PDC_Updates version */
	var $version = '2.4';
	...
	*  request
	*
	*  Makes a request to the PDC connect server.
	*
	*  @date	8/4/17
	*  @since	5.5.10
	*
	...
	*/
	...
	function get_plugin_info( $id = '', $force_check = false ) {
		...
		// var
		$transient_name = 'pdc_plugin_info_' . $id;
	...
	*/
	...
	function get_plugin_updates( $force_check = false ) {
		...
		// var
		$transient_name = 'pdc_plugin_updates';
		...
		$post = array(
			...
			'pdc'			=> wp_json_encode(array(
				'pdc_version'	=> get_option('pdc_version'),
				'pdc_pro'		=> (defined('PDC_PRO') && PDC_PRO),
			)),
		);
	...
	*/
	...
	function refresh_plugins_transient() {
		...
		delete_transient('pdc_plugin_updates');
	}
	
	/*
	*  modify_plugins_transient
	*
	*  Called when WP updates the 'update_plugins' site transient. Used to inject PDC plugin update info.
	*
	*  @date	16/01/2014
	*  @since	5.0.0
	*
...
*  pdc_updates
*
*  The main function responsible for returning the one true pdc_updates instance to functions everywhere.
*  Use this function like you would a global variable, except without needing to declare the global.
*
*  Example: <?php $pdc_updates = pdc_updates(); ?>
*
*  @date	9/4/17
*  @since	5.5.12
*
...
function pdc_updates() {
	global $pdc_updates;
	if( !isset($pdc_updates) ) {
		$pdc_updates = new PDC_Updates();
	}
	return $pdc_updates;
}
...
/*
*  pdc_register_plugin_update
*
*  Alias of pdc_updates()->add_plugin().
*
*  @type	function
*  @date	12/4/17
*  @since	5.5.10
...
function pdc_register_plugin_update( $plugin ) {
	pdc_updates()->add_plugin( $plugin );	
}
