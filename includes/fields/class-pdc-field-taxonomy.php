...
<div <?php pdc_esc_attr_e($div); ?>>
	...
	<div class="pdc-actions -hover">
		<a href="#" class="pdc-icon -plus pdc-js-tooltip small" data-name="add" title="<?php echo esc_attr($taxonomy->labels->add_new_item); ?>"></a>
	</div>
	<?php endif;
	...
	*/
	...
	function render_field_checkbox( $field ) {
		...
		// include walker
		pdc_include('includes/walkers/class-pdc-walker-taxonomy-field.php');
		
		
		// vars
		$args = array(
			'taxonomy'     		=> $field['taxonomy'],
			'show_option_none'	=> sprintf( _x('No %s', 'No terms', 'pdc'), strtolower($taxonomy_obj->labels->name) ),
			'hide_empty'   		=> false,
			'style'        		=> 'none',
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		pdc_render_field_setting( $field, array(
			...
			'type'			=> 'select',
			'name'			=> 'taxonomy',
			'choices'		=> pdc_get_taxonomy_labels(),
		));
	...
	function ajax_add_term() {
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			wp_send_json_error(array(
				'error'	=> sprintf( __('User unable to add new %s', 'pdc'), $taxonomy_label )
			));
		}
		...
		?><p class="pdc-submit">
			<button class="pdc-submit-button button button-primary" type="submit"><?php _e("Add", 'pdc'); ?></button>
		</p>
		</form><?php
...
// initialize
pdc_register_field_type( 'pdc_field_taxonomy' );
