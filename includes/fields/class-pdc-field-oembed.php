...
if( ! class_exists('pdc_field_oembed') ) :

class pdc_field_oembed extends pdc_field {
	...
	*/
	...
	function initialize() {
		...
		// extra
		add_action('wp_ajax_pdc/fields/oembed/search',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_pdc/fields/oembed/search',	array($this, 'ajax_query'));
	...
	*/
	...
	function render_field( $field ) {
		...
		// <strong><?php _e("Error.", 'pdc'); </strong> _e("No embed found for the given URL.", 'pdc');
...
<div <?php pdc_esc_attr_e($atts) ?>>
	...
	<?php pdc_hidden_input(array( 'class' => 'input-value', 'name' => $field['name'], 'value' => $field['value'] )); ?>
	
	<div class="title">
		<?php pdc_text_input(array( 'class' => 'input-search', 'value' => $field['value'], 'placeholder' => __("Enter URL", 'pdc'), 'autocomplete' => 'off'  )); ?>
		<div class="pdc-actions -hover">
			<a data-name="clear-button" href="#" class="pdc-icon -cancel grey"></a>
		</div>
	</div>
	...
	<div class="canvas">
		...
		<i class="pdc-icon -picture hide-if-value"></i>
	</div>
...
// initialize
pdc_register_field_type( 'pdc_field_oembed' );
