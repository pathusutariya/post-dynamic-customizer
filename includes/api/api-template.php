...
function have_rows( $selector, $post_id = false ) {
	...
	if( $new_parent_loop || $new_child_loop ) {
		...
		if( !pdc_is_array($value) ) return false;
		
		
		// allow for non repeatable data (group)
		if( pdc_get_field_type_prop($field['type'], 'have_rows') === 'single' ) {
			$value = array( $value );
		}
		
		
		// add loop
		$active_loop = pdc_add_loop(array(
			'selector'	=> $selector,
...
function get_row( $format = false ) {
	...
	if( $format ) {
		...
		if( pdc_get_field_type_prop($field['type'], 'have_rows') === 'single' ) {
			...
			// format value
			$value = pdc_format_value( $value, $loop['post_id'], $field );
		...
		// multiple rows
			...
			$value = pdc_format_value( $loop['value'], $loop['post_id'], $field );
