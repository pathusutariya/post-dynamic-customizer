...
function pdc_get_taxonomies( $args = array() ) {
	...
	if( isset($args['post_type']) ) {
		$taxonomies = pdc_get_taxonomies_for_post_type($args['post_type']);
	}
	
	// filter
	$taxonomies = apply_filters('pdc/get_taxonomies', $taxonomies, $args);
...
/**
*  pdc_get_taxonomies_for_post_type
*
...
*  @return	array
*/
function pdc_get_taxonomies_for_post_type( $post_types = 'post' ) {
...
function pdc_get_taxonomy_labels( $taxonomies = array() ) {
...
function pdc_get_grouped_terms( $args ) {
	...
	// vars
	$taxonomies = pdc_get_taxonomy_labels( pdc_get_array($args['taxonomy']) );
	$is_single = (count($taxonomies) == 1);
	
	// specify exact taxonomies required for _pdc_terms_clauses() to work.
	$args['taxonomy'] = array_keys($taxonomies);
	
	// add filter to group results by taxonomy
	if( !$is_single ) {
		add_filter('terms_clauses', '_pdc_terms_clauses', 10, 3);
	}
	...
	if( !$is_single ) {
		remove_filter('terms_clauses', '_pdc_terms_clauses', 10, 3);
	}
...
function _pdc_terms_clauses( $pieces, $taxonomies, $args ) {
...
/**
*  pdc_get_pretty_taxonomies
*
*  Deprecated in favor of pdc_get_taxonomy_labels() function.
*
*  @date		7/10/13
*  @since		5.0.0
*  @deprecated	5.7.2
...
function pdc_get_pretty_taxonomies( $taxonomies = array() ) {
	return pdc_get_taxonomy_labels( $taxonomies );
}
...
function pdc_get_term( $term_id, $taxonomy = '' ) {
...
*  @return	string
*/
function pdc_decode_term( $string ) {
...
*  @return	array
*/
function pdc_get_encoded_terms( $values ) {
...
*  @return	array
*/
function pdc_get_choices_from_terms( $terms, $format = 'term_id' ) {
	
	// vars
	$groups = array();
	
	// get taxonomy lables
	$labels = pdc_get_taxonomy_labels();
	
	// convert array of encoded strings to terms
	$term = reset($terms);
	if( !$term instanceof WP_Term ) {
		$terms = pdc_get_encoded_terms( $terms );
	}
	
	// loop over terms
	foreach( $terms as $term ) {
		$group = $labels[ $term->taxonomy ];
		$choice = pdc_get_choice_from_term( $term, $format );
...
/**
*  pdc_get_choices_from_grouped_terms
*
...
*  @return	array
*/
function pdc_get_choices_from_grouped_terms( $value, $format = 'term_id' ) {
...
*  @return	array
*/
function pdc_get_choice_from_term( $term, $format = 'term_id' ) {
	
	// vars
	$id = $term->term_id;
	$text = pdc_get_term_title( $term );
	
	// return format
	if( $format == 'slug' ) {
		$id = pdc_encode_term($term);
	}
